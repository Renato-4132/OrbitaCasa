...
def _genera_testo_estratto_mensile(self):
    ...
    for d, voci in self.spese.items():
        ...
        if d_date.year == anno and d_date.month == mese:
            for v in voci:
                if len(v) < 4:
                    continue
                desc, imp, tipo, cat = str(v[1]), float(v[2]), str(v[3]), str(v[0])
                if tipo == "Entrata":
                    tot_e += imp
    ...
    lines.append("─" * 24)
    lines.append("")
    lines.append(f"🟢 ENTRATE: {tot_e:.2f} €")
    lines.append(f"🔴 USCITE:  {tot_u:.2f} €")
    emoji_saldo = "💰" if saldo >= 0 else "⚠️"
    lines.append(f"{emoji_saldo} SALDO: {saldo:.2f} €")
    lines.append("")
    if cat_entrate:
        ...
        for cat, data in sorted(cat_entrate.items(), key=lambda x: x[1]["totale"], reverse=True):
            ...
            lines.append(f"   Ricevuto: {data['totale']:.2f} €  Voci: {data['voci']}")
    ...
    if cat_uscite:
        ...
        for cat, data in sorted(cat_uscite.items(), key=lambda x: x[1]["totale"], reverse=True):
            ...
            barra = "■" * quadratini + "□" * (10 - quadratini)
            lines.append(f"🟥  {cat[:16]} ({p:.1f}%)")
            lines.append(f"   Speso:    {data['totale']:.2f} €  Voci: {data['voci']}")
            lines.append(f"   [{barra}]")
            lines.append("┈" * 22)
        lines.append("")
    ...
    for m in movimenti:
        ...
        lines.append(f"{segno_emoji} {m[4]:.2f} €  {data_str}")
...
def _genera_testo_estratto_annuale(self):
    ...
    for d, voci in self.spese.items():
        ...
        if d_date.year == anno:
            for v in voci:
                if len(v) < 4:
                    continue
                imp, tipo, cat = float(v[2]), str(v[3]), str(v[0])
                if tipo == "Entrata":
                    tot_e += imp
                    cat_entrate[cat] = cat_entrate.get(cat, {"voci": 0, "totale": 0.0})
                    cat_entrate[cat]["voci"] += 1
    ...
    lines.append("─" * 31)
    lines.append("")
    lines.append(f"🟢 TOTALE ENTRATE ANNO: {tot_e:.2f} €")
    lines.append(f"🔴 TOTALE USCITE ANNO:  {tot_u:.2f} €")
    emoji_saldo = "💰" if saldo >= 0 else "⚠️"
    lines.append(f"{emoji_saldo} SALDO PROGRESSIVO:   {saldo:.2f} €")
    lines.append("")
    if cat_entrate:
        lines.append("💰 ENTRATE PER CATEGORIA")
        lines.append("─" * 31)
