        ...
        def salva():
            ...
            try:
                ...
                if idx + 1 < len(items):
                    ...
                    next_cons = round(max(0.0, next_att_f - nuovo_att), 2)
                    self.trees[utenza].item(items[idx + 1], values=(next_mese, nuovo_att, next_att_f, next_cons))
                anno_sel = anno_var.get()
                righe = [
                ...
                if a < p:
                    ...
                    def ok():
                        ...
                        if idx + 1 < len(items):
                            ...
                            next_mese, _, next_att, _ = next_item['values']
                            next_att_f = float(next_att)
                            next_cons = round(max(0.0, next_att_f - a), 2)
