    ...
    def aggiorna(*_):
        ...
        for data_obj, voci in sorted(self.spese.items()):
            ...
            for voce in voci:
                ...
                if tipo == "Entrata":
                    tot_e += importo
                    righe.append((data_obj, data_str, cat, desc, f"+{importo:.2f} €", "", "entrata", nome_conto_em, metodo_voce, ora_voce, tag_voce))
                else:
                    tot_u += importo
                    righe.append((data_obj, data_str, cat, desc, "", f"-{importo:.2f} €", "uscita", nome_conto_em, metodo_voce, ora_voce, tag_voce))
