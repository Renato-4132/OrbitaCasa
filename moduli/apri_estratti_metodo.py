...
def apri_estratti_metodo(self, metodo=None, mese=None, anno=None, conto=None):
    ...
    cols = ("data", "categoria", "descrizione", "entrata", "uscita", "conto", "metodo")
    ...
    def aggiorna(*_):
        ...
        for data_obj, voci in sorted(self.spese.items()):
            ...
            for voce in voci:
                ...
                if tipo == "Entrata":
                    tot_e += importo
                    righe.append((data_obj, data_str, cat, desc, f"+{importo:.2f} €", "", "entrata", nome_conto_em, metodo_voce, ora_voce))
                else:
                    tot_u += importo
                    righe.append((data_obj, data_str, cat, desc, "", f"-{importo:.2f} €", "uscita", nome_conto_em, metodo_voce, ora_voce))
        righe.sort(key=lambda r: (r[0], r[9]), reverse=True)
        for r in righe:
            tree.insert("", "end", values=(r[1], r[2], r[3], r[4], r[5], r[7], r[8]), tags=(r[6],))
        saldo = tot_e - tot_u
