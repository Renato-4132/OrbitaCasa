...
def time_machine(self):
    ...
    def aggiorna_interfaccia(*_):
        ...
        for cat, dati in top_cat:
            ...
            txt = f"{cat.title()} – {dati['count']}×, Risparmio: {_fmt_it(dati['risparmio'])} € (Uscite: {_fmt_it(dati['uscite'])} - Entrate: {_fmt_it(dati['entrate'])})"
            chk = tk.Checkbutton(
                    destra, 
                    text=txt, 
                    variable=var,
    ...
    def esegui_simulazione():
        ...
        for cat, n, usc, ent, risp in risultati:
            lines.append(f"{cat.title():<25} {n:>4}   {_fmt_it(usc):>12}   {_fmt_it(ent):>12}   {_fmt_it(risp):>14}")
            totale += risp
        lines.append("─" * 77)
        lines.append(f"\nRisparmio totale teorico: {_fmt_it(totale)} € (Uscite - Entrate delle categorie selezionate)")
