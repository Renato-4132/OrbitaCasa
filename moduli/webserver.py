    ...
    def genera_html_categorie(categorie_totals, raw_dettaglio, prefix, counts_dict):
        ...
        for cat, totale in sorted(categorie_totals.items()):
            ...
            dettaglio_items_html = ''.join(
                f'<li class="det-item"><span class="det-text">{data.strftime("%d-%m-%Y")}{" — " + desc if desc else ""}</span><span class="det-amt {color_class}">€{imp:.2f}</span></li>'
            ...
            <li class="cat-item">
                <div class="cat-summary">
                    {arrow_button_html}
                    <span class="cat-name">{cat} <small>({num_ops})</small></span>
                    <span class="cat-total {color_class}">€{totale:.2f}</span>
                </div>
...
<style>
    ...
    .cat-name {{ flex:1; font-size:0.88em; color:var(--text); }}
    ...
    <div class="stats-card">
        ...
        <div class="stats-grid">
            <div class="stat-box">
                ...
                <span class="stat-val amt-income">+€{entrate:.2f}</span>
            </div>
            <div class="stat-box">
                <span class="stat-label">Uscite</span>
                <span class="stat-val amt-expense">-€{uscite:.2f}</span>
            </div>
        </div>
        <div class="saldo-row">
            <div class="saldo-label">Saldo attuale</div>
            <div class="saldo-val" style="color:{saldo_colore};">€{saldo:.2f}</div>
        </div>
    </div>
...
def modifica_voce_form(self, params):
    ...
    try:
        ...
        conti_options_mod = '<option value="">(nessuno)</option>\n' + "\n".join(
            ...
            f'{c.get("nome","")}\u2002(\u20ac{float(c.get("saldo",0)):.2f})</option>'
