...
def _crea_flask_app(self):
    ...
    def salva_utenza_web():
        ...
        for i, mese in enumerate(MESI_NOMI):
            ...
            righe.append([mese, prec, att, cons])
...
def start_web_server(self):
    ...
    # NOTA: PORTA e' una globale mutabile (viene incrementata se la porta e'
    # occupata) e deve restare sincronizzata con __main__, quindi qui si
    # legge/scrive sempre tramite _app.PORTA invece di una copia locale.
    ...
    while getattr(self, "_server_running", True):
        self.server = None
        try:
            # Verifica preventiva della porta: se occupata la incrementiamo
            # PRIMA di chiamare make_server, cosi' Werkzeug non tenta mai il
            # bind su una porta gia' in uso e non stampa il suo messaggio
            # interno (senza timestamp) su stderr.
            ...
            # Fallback: se nonostante il controllo preventivo il bind fallisce
            # comunque (race condition, porta presa nel frattempo), incrementiamo
            # e riproviamo al giro successivo del while esterno.
            _app.PORTA += 1
    ...
    for cat, voci in sorted(risultati_categorizzati.items()):
        ...
        for v in voci:
            ...
            <li class="voce-item">
                ...
                <div class="voce-body">
                    <span class="voce-imp" style="color:{colore_tipo}">{simbolo}€{v['imp']:.2f}</span>
        ...
        <div class="cat-block">
            <button class="cat-toggle" onclick="toggleCategoria(this)">
                ...
                <span class="cat-totale" style="color:{colore_tot}">{simbolo_tot}€{abs(totale_cat):.2f} · {len(voci)} voci</span>
            </button>
    ...
    <div class="riepilogo">
        <div class="riepilogo-grid">
            <div class="riepilogo-item">
                <small>Entrate</small>
                <b style="color:var(--green)">€{entrate_totali:.2f}</b>
            </div>
            <div class="riepilogo-item">
                <small>Uscite</small>
                <b style="color:var(--red)">€{uscite_totali:.2f}</b>
            </div>
            <div class="riepilogo-item">
                <small>Saldo</small>
                <b style="color:{colore_saldo}">€{saldo:.2f}</b>
            </div>
        </div>
    </div>
    {schede_html}
...
def html_form(self):
    ...
    if not movimenti_mese_corrente:
        ...
        for mov in movimenti_mese_corrente[:10]:
            ...
            <div class="mov-item">
                ...
                <div class="mov-amount" style="color:{colore}">{segno}€{mov["imp"]:.2f}</div>
            </div>"""
    ...
    try:
        ...
        conti_options = "\n".join(
            f'<option value="{c.get("nome","")}" {"selected" if c.get("nome","") == _conto_princ else ""}>{c.get("nome","")}\u2002(\u20ac{float(c.get("saldo",0)):.2f})</option>'
        ...
        <div id="summaryContent" class="saldo-content">
            <div class="saldo-grid">
                <div class="saldo-item">
                    <small>Entrate</small>
                    <b style="color:var(--green)">€{entrate_mese:.2f}</b>
                </div>
                <div class="saldo-item">
                    <small>Uscite</small>
                    <b style="color:var(--red)">€{uscite_mese:.2f}</b>
                </div>
                <div class="saldo-item">
                    <small>Saldo</small>
                    <b style="color:{saldo_colore}">€{saldo_mese:.2f}</b>
                </div>
            </div>
...
<script>
    ...
    function renderPersonali(d) {{
        ...
        d.personali.forEach((p, idx) => {{
            ...
            div.innerHTML  = `
                <div class="pers-name">⚖️ ${{p.nome}}</div>
                ...
                chartsPers[chartId] = new Chart(ctx, {{
                    ...
                    options:{{
                        ...
                        scales:{{
                            ...
                            y:{{ticks:{{color:"#888",font:{{size:9}},callback:v=>"€"+v}},grid:{{color:"rgba(255,255,255,0.04)"}}}}
                        }}
                    }}
                }});
            }} else {{
        ...
        for superm in supermercati:
            ...
            if articoli:
                for art in sorted(articoli, key=lambda x: x.get('nome','')):
                    ...
                    try: pn = f"{float(art.get('prezzo','0')):.2f}"
                    except: pn = art.get("prezzo","N/D")
                    promo_attiva = art.get("promo", False)
                    try: pp = float(art.get("prezzo_promo","0"))
                    except: pp = 0
                    promo_cell = f"<span class='promo'>€{pp:.2f}</span>" if promo_attiva and pp > 0 else "—"
...
def html_gestione_categorie(self):
    ...
    categorie_options = "".join(
        f"<option value='{cat}' data-budget='{self.budget_categorie.get(cat, 0):.2f}'>{cat}</option>"
    ...
    function buildDonutLegend(legendId, data) {{
        ...
        legendEl.innerHTML = labels.map((lbl, i) => {{
            ...
            return `<div class="legend-item">
                ...
                <span class="legend-val">€${{Number(vals[i]).toFixed(2)}}</span>
    ...
    function renderChart(tabName) {{
        ...
        if (isDonut) {{
            ...
            opts.scales = {{
                ...
                y: {{ ticks: {{ color: cc.tick, font: {{ family: 'DM Sans' }} }}, grid: {{ color: cc.grid }} }}
            }};
        }}
        ...
        for d, idx, cat, desc, imp, tipo in current_month_expenses:
            ...
            <div class="op-item">
                <div class="op-summary" onclick="toggleVisibility('{details_id}', this)">
                    ...
                    <span class="op-amt" style="color:{colore_imp};">{segno}€{imp:.2f}</span>
                </div>
    ...
    <div class="summary-card">
        ...
        <div class="summary-grid">
            <div class="sg-box">
                ...
                <span class="sg-val" style="color:var(--green);">+€{tot_entrate:.2f}</span>
            </div>
            <div class="sg-box">
                <span class="sg-label">Uscite</span>
                <span class="sg-val" style="color:var(--red);">-€{tot_uscite:.2f}</span>
            </div>
        </div>
        <div class="saldo-row">
            <div class="saldo-lbl">Saldo attuale</div>
            <div class="saldo-num" style="color:{colore_saldo};">€{saldo_mese:.2f}</div>
        </div>
    </div>
    {schede_html}
    <a href="/" class="btn-home">🏠 Torna alla Home</a>
...
def aggiungi_voce_web(self, voce):
    ...
    self.carica_db_web()
    _nome_c_add = voce.get("conto", "")
    if _nome_c_add and _nome_c_add != "(nessuno)":
        try:
            _imp_add = round(float(voce.get("importo", 0)), 2)
            _tipo_add = voce.get("tipo", "Uscita")
            _cat_add = voce.get("categoria", "")
            _desc_add = voce.get("descrizione", "")
            with open(PORTAFOGLIO_BANCARIO, "r", encoding="utf-8") as _pf:
                _db_p = json.load(_pf)
            for _c in _db_p.get("conti", []):
                if _c.get("nome") == _nome_c_add:
                    _segno = -1 if _tipo_add == "Uscita" else 1
                    _c["saldo"] = round(float(_c.get("saldo", 0)) + _segno * _imp_add, 2)
                    ...
                    _db_p.setdefault("trasferimenti", []).append({
                        "id": f"t{_i}",
                        "data": d_obj.strftime("%d-%m-%Y"),
                        "da": _c["id"] if _tipo_add == "Uscita" else "__spese__",
                        "a":  "__spese__" if _tipo_add == "Uscita" else _c["id"],
                        "importo": _imp_add,
                        "note": f"{_cat_add} – {_desc_add}".strip(" –")
                    })
                    break
            with open(PORTAFOGLIO_BANCARIO, "w", encoding="utf-8") as _pf:
                json.dump(_db_p, _pf, indent=2, ensure_ascii=False)
        except Exception as _ex:
            print(f"[aggiungi_voce_web] Errore aggiornamento portafoglio: {_ex}")
    self.refresh_gui()
...
def analizza_pdf_web(self, pdf_bytes, filename_originale="documento.pdf"):
    ...
    self.spese.setdefault(data_oggi, []).append((categoria, desc_spesa, importo, direzione))
    self.save_db()
    ...
    with open(LOG_IMPORTAZIONI, "a", encoding="utf-8") as log:
        log.write(
            ...
            f"{importo:>10.2f} € | {direzione:<7} | {categoria}\n"
        )
    return {
        "ok":        True,
        "azienda":   azienda,
