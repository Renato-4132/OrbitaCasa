...
def apri_studio(self):
    ...
    _parse_it_float      = _app._parse_it_float
    ...
    def _fmt_num(val):
        try:
            return format(float(val), ".2f").replace(".", ",")
        except Exception:
            return ""
    ...
    win.withdraw()
    _ragione_sociale = emittente.get("ragione_sociale", "").strip()
    win.title(f"MyBusiness — {_ragione_sociale}" if _ragione_sociale else "MyBusiness — Clienti, Fatture e Cassa")
    ...
    _add_tab(tab_cli,  "utenti",     "Clienti")
    _add_tab(tab_app,  "calendario", "Agenda")
    _add_tab(tab_pre,  "lavoro",     "Prestazioni")
    _add_tab(tab_fat,  "fattura_ai", "Fatture / Preventivi")
    _add_tab(tab_cas,  "saldo",      "Cassa")
    _add_tab(tab_mag,  "archivia",   "Magazzino")
    _add_tab(tab_emi,  "studio",     "La Mia Azienda")
    tab_close = tk.Frame(nb, bg=self.COLOR_BACKGROUND)
    _add_tab(tab_close, "chiudi", "Chiudi")
    def _check_close(e):
        try:
    ...
    def _row(fw, label, row, col=0):
        tk.Label(fw, text=label, bg=self.COLOR_WIDGET_BG, fg=self.TEXT_COLOR,
    ...
    def _build_agenda():
        ...
        def _draw_cal(event=None):
            ...
            for i, gn in enumerate(GIORNI_IT):
                cx = i * col_w + col_w / 2
                fg = self.COLOR_RED if i >= 5 else self.TEXT_COLOR
            ...
            for g in range(num_giorni):
                ...
                cal_cv.create_rectangle(x0, y0, x1, y1,
                                        ...
                                        if is_sel or is_oggi else self.TEXT_COLOR)
        ...
        def _form(prefill=None):
            ...
            if prefill:
                for e,k in [(e_nome,"nome"),(e_cat,"categoria"),
                            (e_note,"note")]:
                    e.insert(0, prefill.get(k,""))
                e_prez.insert(0, _fmt_num(prefill.get("prezzo","")) if prefill.get("prezzo","") != "" else "")
            ...
            def _salva():
                ...
                if not nome:
                    self.show_toast("Nome obbligatorio."); return
                try:    prezzo = _parse_it_float(e_prez.get())
    ...
    def _build_fatture():
        ...
        def _next_num(tipo):
            ...
            filtrati = [f for f in fatture
                        if f.get("tipo_doc","Fattura") == tipo
                        and f.get("numero","").split("/")[-1] == str(anno)]
        ...
        def _form(prefill=None):
            ...
            def _upd_subtot(dv, qv, pv, iv, sv):
                    try:
                            q = _parse_it_float(qv.get())
                            p = _parse_it_float(pv.get())
                            i = float(iv.get() or 0)
                            sv.set(_fmt_num(q*p*(1+i/100)))
                    except Exception:
                            sv.set("0,00")
            def _add_riga(desc="", qty=1, prezzo=0.0, iva=22):
                ...
                qv = tk.StringVar(value=f"{float(qty or 0):g}".replace(".", ","))
                pv = tk.StringVar(value=_fmt_num(prezzo or 0))
                ...
                def _autocomplete(*_, entry=e_desc, dvar=dv, pvar=pv, ivar=iv):
                    ...
                    lb = tk.Listbox(pop_ac, bg=self.COLOR_WIDGET_BG, fg=self.TEXT_COLOR,
                                    selectbackground=self.COLOR_HIGHLIGHT, font=("Arial",9),
                                    relief="flat", bd=1, width=50, height=min(6, len(risultati)))
                    lb.pack()
                    for c in risultati:
                        lb.insert("end", f"{c['nome']}  —  {_fmt_num(c['prezzo'])} €")
                    def _scegli(evt):
                        idx = lb.curselection()
                        if not idx:
                            return
            ...
            def _scegli_da_magazzino():
                ...
                for m in sorted(magazzino, key=lambda x: x.get("nome","").lower()):
                    tree_p.insert("", "end", iid=m.get("id"), values=(
                        m.get("nome",""), 
                        f"{m.get('prezzo_unit',0):.2f}".replace(".", ",") + " €", 
            ...
            def _salva():
                ...
                for dv, qv, pv, iv in righe_vars:
                    ...
                    try: q = _parse_it_float(qv.get(), 1)
                    except: q = 1
                    try: p = _parse_it_float(pv.get(), 0)
                    except: p = 0
        ...
        def _genera_pdf(doc=None, path_out=None):
            ...
            try:
                ...
                for i, rig in enumerate(righe):
                    ...
                    _txt(page, CX["prez_r"], y+10, _fmt(prez),        size=8, align="right")
                    _txt(page, CX["iva_r"],  y+10, f"{iva_r:.0f}%",   size=8, align="right")
                    _txt(page, CX["imp_r"],  y+10, _fmt(sub),         size=8, align="right")
                    y += ROW_H
                y += 10
...
"""
    ...
    def _build_cassa():
        ...
        def _trasferisci_spesa():
            ...
            CATEGORIA_STUDIO = "BizTrack"
            if CATEGORIA_STUDIO not in self.categorie:
                self.categorie.append(CATEGORIA_STUDIO)
                self.categorie_tipi[CATEGORIA_STUDIO] = "Entrata"
                self.aggiorna_combobox_categorie()
            try:
                with open(PORTAFOGLIO_BANCARIO, "r", encoding="utf-8") as _pf:
                    _db_p_cassa = json.load(_pf)
                _conti_cassa = [c.get("nome", "?") for c in _db_p_cassa.get("conti", [])]
                _conto_principale_cassa = next(
                    (c.get("nome","") for c in _db_p_cassa.get("conti",[]) if c.get("principale")),
                    "(nessuno)"
                )
            except Exception:
                _conti_cassa = []
                _conto_principale_cassa = "(nessuno)"
            popup = tk.Toplevel(win, bg=self.COLOR_TOPLEVEL)
            ...
            _lbl(0, "Data:")
            frm_data = tk.Frame(f, bg=self.COLOR_TOPLEVEL)
            frm_data.grid(row=0, column=1, sticky="w", pady=4)
            entry_data = ttk.Entry(frm_data, width=12)
            entry_data.pack(side="left")
            entry_data.insert(0, datetime.date.today().strftime("%d/%m/%Y"))
            _mk_cal_btn(frm_data, entry_data)
            _lbl(1, "Importo:")
            ...
            _lbl(3, "Conto:")
            conto_var = tk.StringVar(value=_conto_principale_cassa)
            cbo_conto = ttk.Combobox(f, textvariable=conto_var,
                                     values=["(nessuno)"] + _conti_cassa,
                                     state="readonly", width=22,
                                     style="Border.TCombobox")
            cbo_conto.grid(row=3, column=1, sticky="w", pady=4)
            _lbl(4, "Tag:")
            tag_var = tk.StringVar(value="#biztrack")
            ttk.Entry(f, textvariable=tag_var, width=22
                              ).grid(row=4, column=1, sticky="w", pady=4)
            _lbl(5, "Metodo:")
            metodo_var = tk.StringVar(value="")
            cbo_metodo = ttk.Combobox(f, textvariable=metodo_var,
                                      values=[""] + METODI_PAGAMENTO,
                                      state="readonly", width=22,
                                      style="Border.TCombobox")
            cbo_metodo.grid(row=5, column=1, sticky="w", pady=4)
            def _conferma():
                    try:
                            data_obj = datetime.datetime.strptime(entry_data.get().strip(), "%d/%m/%Y").date()
                    except ValueError:
                            self.show_toast("Data non valida (GG/MM/AAAA)."); return
                    desc = desc_var.get().strip() or note
                    nome_conto_scelto = conto_var.get()
                    conto_voce = nome_conto_scelto if nome_conto_scelto and nome_conto_scelto != "(nessuno)" else ""
                    tag_lista = self._normalizza_tags(tag_var.get()) if hasattr(self, "_normalizza_tags") else [t for t in tag_var.get().replace(",", " ").split() if t]
                    if data_obj not in self.spese:
                            self.spese[data_obj] = []
                    self.spese[data_obj].append(SpesaEntry.nuova(
                        CATEGORIA_STUDIO, desc, imp, tipo_spesa,
                        conto=conto_voce,
                        metodo_pagamento=metodo_var.get(),
                        hashtag=tag_lista
                    ))
                    self.save_db()
                    mov["trasferito"] = True
        ...
        def _form(prefill=None):
            ...
            if prefill:
                ...
                e_imp.insert(0, _fmt_num(prefill.get("importo","")) if prefill.get("importo","") != "" else "")
            ...
            def _salva():
                ...
                try:
                    ...
                    self.show_toast("Data non valida (GG/MM/AAAA)."); return
                try: imp = _parse_it_float(e_imp.get())
        ...
        def _form(prefill=None):
            ...
            if prefill:
                for e,k in [(e_nome,"nome"),(e_cat,"categoria"),(e_note,"note")]:
                    e.insert(0,prefill.get(k,""))
                e_qty.insert(0, _fmt_num(prefill.get("quantita","")) if prefill.get("quantita","") != "" else "")
                e_unit.set(     prefill.get("unita","pz"))
                e_sog.insert(0, _fmt_num(prefill.get("soglia","0")))
                e_pu.insert(0,  _fmt_num(prefill.get("prezzo_unit","")) if prefill.get("prezzo_unit","") != "" else "")
            else:
                e_unit.set("pz")
            def _salva():
                nome = e_nome.get().strip()
                if not nome: self.show_toast("Nome obbligatorio."); return
                try:    qty    = _parse_it_float(e_qty.get())
                except: self.show_toast("Quantità non valida."); return
                try:    soglia = _parse_it_float(e_sog.get())
                except: soglia = 0
                try:    pu     = _parse_it_float(e_pu.get())
                except: pu     = 0
                record = {
                    "id":          prefill["id"] if prefill else _new_id(),
                    "nome":        nome,
        ...
        def _esporta_pdf():
            ...
            try:
                ...
                for i, m in enumerate(sorted(magazzino,
                    ...
                    _txt(page, CX["val_r"],  y+10, _fmt_num(valore),     size=8, align="right")
