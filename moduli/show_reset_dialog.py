...
def _elenco_file_variabili(_app):
    ...
    voci_raw = [
        ...
        ("Tag dei Movimenti",                 "Tag assegnati ai movimenti (tags_db.json)", "TAGS_DB", False),
...
def show_reset_dialog(self):
    ...
    dot = tk.Canvas(header, width=10, height=10, bg=self.COLOR_BACKGROUND, highlightthickness=0)
    dot.create_oval(0, 0, 10, 10, fill=self.COLOR_HIGHLIGHT, outline="")
    dot.pack(side="left", padx=(16, 8), pady=16)
    tk.Label(header, text="RESET DATI", bg=self.COLOR_HEADER_BG, fg=self.COLOR_HEADER,
    ...
    def _aggiorna_contatore(*_a):
        selezionati = sum(1 for _r, v, *_ in righe if v.get())
        lbl_count.config(text=f"{selezionati} selezionati su {len(righe)}")
    ...
    def _popola(filtro=""):
        ...
        for etichetta, descrizione, percorso, is_dir in voci:
            ...
            var.trace_add("write", _aggiorna_contatore)
            chk = tk.Checkbutton(
    ...
    def _esegui_reset():
        selezionati = [(e, p, d) for _r, v, e, _de, p, d in righe if v.get()]
        if not selezionati:
        ...
        if errori:
            self.show_custom_warning("Errore Reset", "Alcuni elementi non sono stati azzerati:\n" + "\n".join(errori))
            return
