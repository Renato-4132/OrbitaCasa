...
from __main__ import API_KEY, GEMINI, PATH_LOCALE, genai_client, _fmt_it
...
def analizza_andamento_ia(self):
    ...
    stringa_categorie = "\n".join([f"   - {c:.<25} {_fmt_it(v):>12}€" for c, v in cat_ordinate.items()])
    ...
    - Entrate Totali: {_fmt_it(e_tot)}€
    - Uscite Totali: {_fmt_it(u_tot)}€
    - Media Uscite Mensile: {_fmt_it(u_tot/mesi_coperti)}€ (calcolata su {mesi_coperti} mesi di dati disponibili)
    ...
    3. Scrivi tutti gli importi in formato italiano (virgola come separatore decimale, punto per le migliaia, es. 1.234,56€) — mai il punto come separatore decimale.
    4. TASK 1: Analisi breve dei dati storici.
    5. TASK 2: PREDIZIONE FINE ANNO. Stima quanto sarà il totale uscite al {fine_anno}.
    6. TASK 3: Suggerimento per chiudere l'anno in attivo.
    """
