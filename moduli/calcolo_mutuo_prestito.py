    ...
    def popola_piano(tree_widget, capitale_iniziale, anni, mesi, rata_base, spese_mensili, tasso_mensile, title_label, ammortamento_extra=0):
        ...
        if ammortamento_extra > 0:
            ...
            tree_widget.insert("", "end", values=(
                "Ammortamento Extra",
                f"{_fmt_it(ammortamento_extra)} €",
                f"{_fmt_it(ammortamento_extra)} €",
                "0.00 €",
                f"{_fmt_it(debito_res)} €"
            ), tags=('extra_row',))
        ...
        for mese in range(1, mesi + 1):
            ...
            tree_widget.insert("", "end", values=(
               label_periodo,
               f"{_fmt_it(rata_base_nuova + spese_mensili)} €",
               f"{_fmt_it(capitale_rata)} €",
               f"{_fmt_it(interessi_rata)} €",
               f"{_fmt_it(debito_res if debito_res > 0.005 else 0.0)} €"
            ))
        totale_rata_pagata = totale_capitale + totale_interessi + (spese_mensili * mesi)
        riepilogo_text = (
            f"Capitale: {_fmt_it(capitale_iniziale)} €\n"
            f"Durata: {anni} anni ({mesi} mesi)\n"
            f"Tasso: {_fmt_it(tasso_mensile * 100 * 12)} %\n"
            f"Metodo: Ammortamento Francese (Rata Costante)\n"
            f"Ammortamento Extra: {_fmt_it(ammortamento_extra)} €\n"
            f"Importo Totale Restituito: {_fmt_it(totale_rata_pagata)} €\n"
            f"Interessi Totali: {_fmt_it(totale_interessi)} €"
        )
        title_label.config(text=riepilogo_text, wraplength=1000)
        tree_widget.insert("", "end", values=("TOTALE", f"{_fmt_it(totale_rata_pagata)} €", f"{_fmt_it(totale_capitale)} €", f"{_fmt_it(totale_interessi)} €", "-"), tags=('total_row',))
    ...
    def aggiorna_simulazione_singola(i):
        ...
        try:
            ...
            if risultati_scenario is not None:
                ...
                lbl_scenari_risultati[i][2].config(text=f"{_fmt_it(risultati_scenario['rata_mensile_totale'])} €")
                lbl_scenari_risultati[i][3].config(text=f"{_fmt_it(risultati_scenario['interessi_totali'])} €")
                costo_totale_cap_int = risultati_scenario['capitale'] + risultati_scenario['interessi_totali']
                lbl_scenari_risultati[i][4].config(text=f"{_fmt_it(costo_totale_cap_int)} €")
        ...
        for i, res in enumerate(risultati):
            if res:
                ...
                tree_analisi.insert("", "end", values=(
                    f"Simulazione {i+1}", 
                    f"{_fmt_it(res['capitale'])} €",
                    f"{res['anni']} anni ({res['mesi']} rate)", 
                    f"{_fmt_it(res['tasso_annuo'])} %",
                    f"{_fmt_it(res['ammortamento_extra'])} €",
                    f"{_fmt_it(res['rata_mensile_totale'])} €", 
                    f"{_fmt_it(res['importo_totale'])} €",
                    f"{_fmt_it(res['interessi_totali'])} €",
                    f"{_fmt_it(risparmio)} €"
                ), tags=('all_rows',))
    ...
    def esporta_dati_con_preview():
        ...
        if has_sim and idx_notebook not in (idx_rata_sost_tab, idx_killer_tab):
            ...
            for i, res in enumerate(self.tutti_i_risultati):
                if res:
                    ...
                    sim_data["Capitale (€)"].append(f"{_fmt_it(res['capitale'])}")
                    sim_data["Durata (anni)"].append(f"{res['anni']}")
                    sim_data["N° Rate"].append(f"{res['mesi']}")
                    sim_data["Scadenza Prevista"].append(data_stringa)
                    sim_data["Tasso (%)"].append(f"{_fmt_it(res['tasso_annuo'])}")
                    sim_data["Spese Incasso (€)"].append(f"{_fmt_it(res['spese_mensili'])}")
                    sim_data["Ammort. Extra (€)"].append(f"{_fmt_it(res['ammortamento_extra'])}")
                    sim_data["Rata Mensile (€)"].append(f"{_fmt_it(res['rata_mensile_totale'])}")
                    sim_data["Interessi Totali (€)"].append(f"{_fmt_it(res['interessi_totali'])}")
                    sim_data["Costo Totale (€)"].append(f"{_fmt_it(res['capitale'] + res['interessi_totali'])}")
                    risparmio = risultati_principali["interessi_totali"] - res["interessi_totali"] if risultati_principali and i > 0 else 0.0
                    sim_data["Risparmio Int. (€)"].append(f"{_fmt_it(risparmio)}")
                else:
        ...
        if idx_notebook == idx_rata_sost_tab:
            if has_rata_sost:
                ...
                contenuto_testo += linea + "\n"
                contenuto_testo += (
                    f"{_fmt_it(rs['rata_disp'], '<25,.2f')} | {rs['anni']:>13} | "
                    f"{_fmt_it(rs['tasso'], '>15,.2f')} | {_fmt_it(rs['spese'], '>17,.2f')} | {_fmt_it(rs['capitale_max'], '>21,.2f')}\n"
                )
                contenuto_testo += linea + "\n"
            ...
            if res_sel:
                ...
                for m in range(1, res_sel['mesi'] + 1):
                    ...
                    contenuto_testo += (
                        f"{etichetta:<30} | "
                        f"{_fmt_it(rata_tot, '>12,.2f')} | "
                        f"{_fmt_it(q_c, '>12,.2f')} | "
                        f"{_fmt_it(q_i, '>12,.2f')} | "
                        f"{_fmt_it(max(0, res_p), '>15,.2f')}\n"
                    )
                contenuto_testo += linea + "\n"
            ...
            if has_killer:
                ...
                contenuto_testo += f"{'─'*117}\n"
                contenuto_testo += f"{' > Debito Residuo Iniziale:':<45} {_fmt_it(capitale_input, '>20,.2f')} €\n"
                if extra_s_input > 0:
                    contenuto_testo += f"{' > Abbattimento Immediato:':<45} {_fmt_it(extra_s_input, '>20,.2f')} €\n"
                contenuto_testo += f"{' > Rata Mensile Attuale:':<45} {_fmt_it(rata_input, '>20,.2f')} €\n"
                contenuto_testo += f"{' > Durata Residua Dichiarata:':<45} {mesi_input:>20} rate\n"
                contenuto_testo += f"{' > Tasso Applicato:':<45} {_fmt_it(tasso_input, '>20,.2f')} %\n"
                contenuto_testo += f"{'─'*117}\n"
                contenuto_testo += " PROIEZIONE SENZA INTERVENTO (PIANO ORIGINARIO)\n"
                contenuto_testo += f"{'─'*117}\n"
                contenuto_testo += f"{' > Scadenza Prevista:':<45} {d_fine_std:>20}\n"
                contenuto_testo += f"{' > Interessi Totali Previsti:':<45} {_fmt_it(int_senza_k, '>20,.2f')} €\n"
                contenuto_testo += f"{' > Totale Montante (Cap+Int):':<45} {_fmt_it(totale_senza_k, '>20,.2f')} €\n"
                contenuto_testo += f"{'─'*117}\n"
                ...
                contenuto_testo += f"{'─'*117}\n"
                contenuto_testo += f"{' > Risparmio Interessi Netto:':<45} {_fmt_it(risparmio_k, '>20,.2f')} €\n"
                contenuto_testo += f"{' > Totale Interessi da Pagare:':<45} {_fmt_it(total_int_k, '>20,.2f')} €\n"
                contenuto_testo += f"{'─'*117}\n"
                ...
                contenuto_testo += f"{'─'*117}\n"
                contenuto_testo += f"{' > TOTALE VERSATO FINALE:':<45} {_fmt_it(total_vers_k, '>20,.2f')} €\n"
                contenuto_testo += f"{'═'*117}\n"
                if hasattr(self, 'killer_annuali') and self.killer_annuali:
                    ...
                    if self.killer_stats and "Anticipo" in str(self.killer_stats[0]['mese']):
                        ...
                        contenuto_testo += (
                            f"{'ANTICIPO':<10} | "
                            f"{oggi.year:<12} | "
                            f"{_fmt_it(anticipo_val, '>18,.2f')} | "
                            f"{_fmt_it(0.0, '>18,.2f')} | "
                            f"{_fmt_it(anticipo_val, '>20,.2f')}\n"
                        )
                    try:
                    ...
                    for ann in self.killer_annuali:
                        ...
                        contenuto_testo += (
                            f"Anno {ann['anno']:<5} | "
                            f"{anno_solare:<12} | "
                            f"{_fmt_it(extra_anno, '>18,.2f')} | "
                            f"{_fmt_it(risparmio, '>18,.2f')} | "
                            f"{_fmt_it(impatto_reale, '>20,.2f')}\n"
                        )
                    if self.killer_stats:
                        ...
                        contenuto_testo += (
                            f"{'CHIUSURA':<10} | "
                            f"{data_fine.year:<12} | "
                            f"{_fmt_it(ultima_r['versato'], '>18,.2f')} | "
                            f"{'---':>18} | "
                            f"{_fmt_it(ultima_r['versato'], '>20,.2f')} (Saldo Finale)\n"
                        )
                    contenuto_testo += "─"*117 + "\n"
                ...
                for row in self.killer_stats:
                    contenuto_testo += (
                        f"{str(row['mese']):<30} | "
                        f"{_fmt_it(row['versato'], '>15,.2f')} | "
                        f"{_fmt_it(row['quota_cap'], '>15,.2f')} | "
                        f"{_fmt_it(row['quota_int'], '>15,.2f')} | "
                        f"{_fmt_it(row['residuo'], '>15,.2f')}\n"
                    )
                contenuto_testo += "═"*117 + "\n"
    ...
    def calcola_ammortamento_killer():
        try:
            ...
            while debito > 0.01 and mese_corrente < 600:
                ...
                tree_k_piano.insert("", "end", values=(
                    label_periodo,
                    f"{_fmt_it(versato_reale)} €", 
                    f"{_fmt_it(abb_eff)} €", 
                    f"{_fmt_it(interessi_mese)} €", 
                    f"{_fmt_it(max(debito,0))} €"
            ...
            testo_risultato = (
                ...
                f"💰 Tot. Versato: {_fmt_it(tot_versato_killer)} €  📉 Interessi: {_fmt_it(tot_interessi_killer)} €  🚀 Risparmio: {_fmt_it(max(0, risparmio_interessi))} €"
            )
            ...
            def comando_popup(e, d_pop, ex_s, lista_dati):
                    ...
                    oggi = datetime.date.today()
                    if ex_s > 0:
                            tree.insert("", "end", values=("ANTICIPO", f"{oggi.year}", f"{_fmt_it(ex_s)} €", "0.00 €", f"{_fmt_it(ex_s)} €"), tags=("evidenza",))
                    for r in lista_dati:
                            data_target = oggi + datetime.timedelta(days=int(r['anno'] * 12 * 30.44))
                            tree.insert("", "end", values=(f"Anno {r['anno']}", f"{data_target.year}", f"{_fmt_it(ex_val)} €", f"{_fmt_it(r['risparmio_rate'])} €", f"{_fmt_it(r['sforzo_netto'])} €"))
                    if self.killer_stats:
                            ...
                            tree.insert("", "end", values=("ESTINZIONE", f"{data_fine.year}", f"{_fmt_it(ultima_riga['versato'])} €", "---", f"{_fmt_it(ultima_riga['versato'])} €"), tags=("saldo",))
    ...
    def calcola_rata_sostenibile():
        try:
            ...
            lbl_rs_risultato.config(
                text=f"💰 Capitale Massimo Finanziabile: {_fmt_it(capitale_max)} €   "
                     f"(rata {_fmt_it(rata_disp)} € su {anni_rs} anni al {_fmt_it(tasso_rs)} %)"
            )
