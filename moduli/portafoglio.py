...
def apri_portafoglio(self):
        if hasattr(self, '_portafoglio_win') and self._portafoglio_win and self._portafoglio_win.winfo_exists():
            self._portafoglio_win.lift()
            self._portafoglio_win.focus_force()
            return
        ...
        menu_dati = tk.Menu(menu_popup, tearoff=0,
                            bg=self.MENU_BG, fg=self.MENU_FG_LIGHT,
                            activebackground=self.MENU_ACT_BG_COLOR,
                            activeforeground=self.MENU_ACT_FG_COLOR)
        menu_popup.add_cascade(label="💾 Dati", menu=menu_dati)
        ...
        menu_dati.bind("<Leave>", _avvia_timer_chiusura_popup)
        menu_dati.bind("<Enter>", _annulla_timer_chiusura_popup)

        def _chiudi_popup_su_spostamento(event=None):
            _chiudi_menu_popup_sicuro()
        win.bind("<Configure>", _chiudi_popup_su_spostamento, add="+")

        header_portafoglio = tk.Frame(win, bg=self.COLOR_BACKGROUND)
        header_portafoglio.pack(side="top", fill="x")
        img_menu_top = self.icone_gui.get("tools")
        btn_menu_top = ttk.Label(header_portafoglio, compound="left", image=img_menu_top, text=" Menu" if img_menu_top else "☰ Menu", background=self.COLOR_WIDGET_BG, foreground=self.TEXT_COLOR, cursor="hand2", padding=(10, 5))
        ...
        def _build_portafoglio():
            ...
            def _sort_porta(col, reverse):
                ...
                try:
                    rows.sort(key=lambda t: float(str(t[0]).replace(".","").replace(",",".").replace("€","").replace("%","").replace("+","").replace(" ","")), reverse=reverse)
                except Exception:
            ...
            for t in dati["titoli"]:
                ...
                tv.insert("", "end", iid=c["ticker"], values=(
                    ...
                    f"{_fmt_it(c['valore_attuale'])}",
                    f"{_fmt_it(c['investito'])}",
                    f"{_fmt_it(c['pl_eur'], '+,.2f')}",
                    f"{c['pl_pct']:+.2f}%",
                ), tags=(tag,))
                tot_inv += c["investito"]
        ...
        def _build_dividendi():
            ...
            for r in sorted(righe, key=lambda x: _porta_key_data(x[0]), reverse=True):
                ...
                tv.insert("", "end", iid=iid, values=(
                    r[0], r[1],
                    f"{_fmt_it(r[2])}", f"{_fmt_it(r[3])}", f"{_fmt_it(r[4])}"))
            tv.insert("", "end", values=("TOTALE", "", "", "", f"{_fmt_it(tot_netto)}"))
            ...
            def _dlg_edit_dividendo(titolo, div):
                ...
                def _calc_netto(*_):
                    try:
                        ...
                        vars_["importo_netto"].set(f"{lordo - ritenuta:.2f}".replace(".", ","))
                    except Exception:
                        pass
        ...
        def _dlg_nuovo_dividendo():
            ...
            for i, (label, key, default) in enumerate(zip(campi_label, campi_key, defaults)):
                tk.Label(dlg, text=label, bg=self.COLOR_BACKGROUND, fg=self.TEXT_COLOR).pack(pady=(10, 0))
                v = tk.StringVar(value=default)
                vars_[key] = v
                if key == "data":
                    frame_data = tk.Frame(dlg, bg=self.COLOR_BACKGROUND)
        ...
        def _anteprima():
            ...
            def _build_testo():
                ...
                for c in titoli_calcolati:
                    lines.append(
                        ...
                        f"{_fmt_it(c['valore_attuale'], '>12,.2f')}  "
                        f"{_fmt_it(c['investito'], '>12,.2f')}  "
                        f"{_fmt_it(c['pl_eur'], '>+12,.2f')}  "
                        f"{c['pl_pct']:>+7.2f}%"
                    )
                lines.append(SEP)
                lines.append(
                    f"{'TOTALE':<10}  {'':24}  {'':>12}  {'':>12}  {'':>12}  "
                    f"{_fmt_it(tot_attuale, '>12,.2f')}  "
                    f"{_fmt_it(tot_investito, '>12,.2f')}  "
                    f"{_fmt_it(pl_tot, '>+12,.2f')}  "
                    f"{pl_pct_tot:>+7.2f}%"
                )
                ...
                for r in sorted(righe_div, key=lambda x: _porta_key_data(x[0]), reverse=True):
                    lines.append(
                        f"{r[0]:<12}  {r[1]:<10}  "
                        f"{_fmt_it(r[2], '>12,.2f')}  {_fmt_it(r[3], '>12,.2f')}  {_fmt_it(r[4], '>12,.2f')}"
                    )
                lines.append(SEP2)
                lines.append(f"{'TOTALE':<12}  {'':10}  {'':>12}  {'':>12}  {_fmt_it(tot_netto, '>12,.2f')}")
                return "\n".join(lines)
            testo = _build_testo()
            popup_width = 1300
            popup_height = 620
            ...
            def _salva_txt():
                ...
                try:
                    ...
                    self.show_custom_warning("Errore", f"Impossibile salvare il TXT: {e}")
            def _salva_pdf():
                import os
                path = filedialog.asksaveasfilename(
        ...
        def _esporta_json():
            ...
            try:
                ...
                self.show_custom_warning("Errore", f"Impossibile esportare il portafoglio: {e}")
        def _importa_json():
        ...
        def _analisi_ia():
            ...
            riepilogo  = (
                f"PORTAFOGLIO:\n"
                f"Investito: €{_fmt_it(tot_investito)} | Valore attuale: €{_fmt_it(tot_attuale)}\n"
                f"P&L: €{_fmt_it(pl_tot, '+,.2f')} ({pl_pct_tot:+.2f}%) | Dividendi: €{_fmt_it(tot_dividendi)} | "
                f"Rendimento reale: €{_fmt_it(rend_reale, '+,.2f')}\n"
            ...
            for c in titoli_calcolati:
                ...
                riepilogo += (
                    ...
                    f"valore €{_fmt_it(c['valore_attuale'])}, P&L {c['pl_pct']:+.2f}%, "
...
def _porta_load(self):
    import json, os
    from __main__ import PORTAFOGLIO_AZIONI
    if not os.path.exists(PORTAFOGLIO_AZIONI):
        return {"impostazioni": {"valuta": "EUR", "soglia_ribilanciamento": 2.0},
                "titoli": []}
    try:
        with open(PORTAFOGLIO_AZIONI, "r", encoding="utf-8") as f:
            return json.load(f)
    except Exception:
        return {"impostazioni": {"valuta": "EUR", "soglia_ribilanciamento": 2.0},
                "titoli": []}

def _porta_save(self, dati):
    import json, os
    from __main__ import PORTAFOGLIO_AZIONI, DB_DIR
    os.makedirs(DB_DIR, exist_ok=True)
    try:
        with open(PORTAFOGLIO_AZIONI, "w", encoding="utf-8") as f:
            json.dump(dati, f, indent=2, ensure_ascii=False)
    except Exception as e:
        if hasattr(self, "show_custom_warning"):
            self.show_custom_warning("Errore salvataggio", f"Impossibile salvare il portafoglio: {e}")
        else:
            print(f"[_porta_save] Errore salvataggio portafoglio: {e}")

def _porta_prezzo_live(self, ticker):
    try:
        import yfinance as yf
        import logging, os, io, contextlib
...
def _porta_calcola_titolo(self, t, prezzo_live=None):
    movimenti = t.get("movimenti", [])
    quantita  = 0.0
    costo_tot = 0.0
    for m in movimenti:
        q = float(m.get("quantita", 0))
        p = float(m.get("prezzo", 0))
        c = float(m.get("commissioni", 0))
        if m.get("tipo") == "acquisto":
            quantita  += q
            costo_tot += q * p + c
        elif m.get("tipo") == "vendita":
            if quantita > 0:
                pmc = costo_tot / quantita if quantita else 0
                quantita  -= q
                costo_tot -= q * pmc
    quantita  = max(quantita, 0.0)
    costo_tot = max(costo_tot, 0.0)
    pmc       = (costo_tot / quantita) if quantita > 0 else 0.0
    dividendi_netti = sum(float(d.get("importo_netto", d.get("importo", 0)))
                          for d in t.get("dividendi", []))
    prezzo = prezzo_live if prezzo_live is not None else float(t.get("ultimo_prezzo", 0))
    valore_attuale = quantita * prezzo
    pl_eur  = valore_attuale - costo_tot
    pl_pct  = (pl_eur / costo_tot * 100) if costo_tot > 0 else 0.0
    return {
        "ticker":          t["ticker"],
        ...
        "pl_eur":          pl_eur,
        "pl_pct":          pl_pct,
        "dividendi_netti": dividendi_netti,
    }
