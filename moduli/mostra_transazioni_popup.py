...
def mostra_transazioni_popup(self, data_filter, title, filtro_desc=None, chiavi_filtro=None, filtro_metodo=None):
    ...
    for data, voci in self.spese.items():
        ...
        for entry in voci:
            ...
            spese_filtrate.append((data, cat_normalized_display, desc, entry_imp, entry_tipo, entry_conto_diretto))
    if not spese_filtrate:
        self.show_custom_info("Nessuna transazione", f"Nessuna transazione trovata per {title}.")
        return
    popup_width, popup_height = 900, 450
    ...
    columns = ("Data", "Categoria", "Descrizione", "Importo", "Tipo", "Conto")
    tree = ttk.Treeview(
        tree_frame, 
        columns=columns, 
        show="headings", 
    ...
    widths = (90, 150, 250, 100, 80, 120)
    anchors = ("center", "w", "center", "e", "center", "center")
    ...
    for d, cat, desc, imp, tipo, conto_diretto in sorted(spese_filtrate, key=lambda x: x[0], reverse=True):
        ...
        tree.insert("", "end", values=(d.strftime("%d-%m-%Y"), cat, desc, f"{imp_formattato} €", tipo, nome_conto_tr), tags=(tag_name,))
