        ...
        for voce in self.spese[giorno]:
            ...
            if voce_cat == cat:
                if not elimina_importi:
                    nuove_spese.append(
                        voce.sostituisci(categoria=self.CATEGORIA_RIMOSSA) if isinstance(voce, SpesaEntry)
                        else (self.CATEGORIA_RIMOSSA,) + voce[1:]
                    )
            else:
...
def mostra_storico_categoria(self, categoria):
    ...
    for d, entries in self.spese.items():
        if d.year == now.year:
            for entry in entries:
                if entry[0] == categoria:
                    if entry[3] == "Entrata":
                        totali_ent[d.month - 1] += entry[2]
                    else:
                        totali_usc[d.month - 1] += entry[2]
...
def open_analisi_categoria(self):
    ...
    def esporta_analisi():
        ...
        btn_salva = ttk.Label(frm, compound="left", image=img_salva, text=" Salva" if img_salva else "💾 Salva", background=self.COLOR_WIDGET_BG, foreground=self.TEXT_COLOR, cursor="hand2", padding=(10, 5))
...
def gruppo_categorie(self):
    ...
    def salva():
        ...
        if not contenuto:
            self.show_toast("Nessun risultato da salvare.")
            return
        now = datetime.date.today()
    ...
    btn_salva_an.bind("<Button-1>", lambda e: salva())
