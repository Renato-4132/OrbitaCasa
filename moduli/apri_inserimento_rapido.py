...
from moduli.modello_spesa import (
    SpesaEntry, campo, METODI_PAGAMENTO_FILTRO,
    VOCE_FILTRO_MOVIMENTI, SEPARATORE_FILTRO_MOVIMENTI, metodo_pagamento_pulito,
)
...
def apri_inserimento_rapido(self, event):
    ...
    metodi_lista = METODI_PAGAMENTO_FILTRO
    ...
    entry_desc_rapida = ttk.Entry(frame, textvariable=var_desc, width=26)
    entry_desc_rapida.grid(row=6, column=1, columnspan=2, sticky="w", pady=5)
    riga_extra = 7
    if _conti_rap:
        ttk.Label(frame, text="Conto:", font=("Arial", 9, "bold")).grid(row=riga_extra, column=0, sticky="w", pady=5)
        ttk.Combobox(frame, textvariable=var_conto_rap,
                     values=["(nessuno)"] + _conti_rap,
                     state="readonly", style="Border.TCombobox", width=25).grid(row=riga_extra, column=1, columnspan=2, sticky="w", pady=5)
        riga_extra += 1
    var_tag_rap = tk.StringVar()
    ttk.Label(frame, text="Tag:", font=("Arial", 9, "bold")).grid(row=riga_extra, column=0, sticky="w", pady=5)
    entry_tag_rapido = ttk.Entry(frame, textvariable=var_tag_rap, width=26)
    entry_tag_rapido.grid(row=riga_extra, column=1, sticky="w", pady=5)
    lbl_hash_rap = ttk.Label(frame, text="#", cursor="hand2", font=("Arial", 12, "bold"))
    lbl_hash_rap.grid(row=riga_extra, column=2, sticky="w", padx=5, pady=5)
    if hasattr(self, 'apri_gestione_tag'):
        lbl_hash_rap.bind("<Button-1>", lambda e: self.apri_gestione_tag())
    _ac_lb_rap = [None]
    def _get_tutti_tag_rap():
        ...
        return sorted(tutti)
    def _chiudi_ac_rap():
        if _ac_lb_rap[0] and _ac_lb_rap[0].winfo_exists():
            _ac_lb_rap[0].destroy()
        _ac_lb_rap[0] = None
    def _on_tag_keyrelease_rap(event):
        if event.keysym in ("Return", "Escape", "Tab"):
            _chiudi_ac_rap()
            return
        testo = entry_tag_rapido.get()
        parole = testo.replace(",", " ").split()
        ultima = parole[-1].lstrip("#") if parole else ""
        if len(ultima) < 1:
            _chiudi_ac_rap()
            return
        suggerimenti = [t for t in _get_tutti_tag_rap() if t.lower().startswith(ultima.lower()) and t.lower() != ultima.lower()]
        if not suggerimenti:
            _chiudi_ac_rap()
            return
        if not _ac_lb_rap[0] or not _ac_lb_rap[0].winfo_exists():
            _ac_lb_rap[0] = tk.Listbox(
                popup,
            ...
            x = entry_tag_rapido.winfo_rootx() - popup.winfo_rootx()
            y = entry_tag_rapido.winfo_rooty() - popup.winfo_rooty() + entry_tag_rapido.winfo_height()
            _ac_lb_rap[0].place(x=x, y=y, width=entry_tag_rapido.winfo_width())
            def _seleziona_rap(e):
                if not _ac_lb_rap[0]:
                    return
                sel = _ac_lb_rap[0].curselection()
                if not sel:
                    return
                scelto = _ac_lb_rap[0].get(sel[0])
                parole_attuali = entry_tag_rapido.get().replace(",", " ").split()
                if parole_attuali:
                    parole_attuali[-1] = scelto
                entry_tag_rapido.delete(0, tk.END)
                entry_tag_rapido.insert(0, " ".join(parole_attuali))
                _chiudi_ac_rap()
            _ac_lb_rap[0].bind("<ButtonRelease-1>", _seleziona_rap)
        _ac_lb_rap[0].delete(0, tk.END)
        _ac_lb_rap[0].config(height=min(5, len(suggerimenti)))
        for s in suggerimenti:
            _ac_lb_rap[0].insert(tk.END, s)
    entry_tag_rapido.bind("<KeyRelease>", _on_tag_keyrelease_rap)
    entry_tag_rapido.bind("<FocusOut>", lambda e: popup.after(300, _chiudi_ac_rap))
    def aggiorna_tutto_desc(event=None):
        testo_puro = entry_desc_rapida.get().strip()
        metodi_simboli = [m.split(" ")[0] for m in metodi_lista if m and m != SEPARATORE_FILTRO_MOVIMENTI]
    ...
    def gestisci_metodo_inserimento(event):
        ...
        if scelta == VOCE_FILTRO_MOVIMENTI:
            ...
            var_metodo.set("")
            return
        _ultimo_metodo_rap["valore"] = metodo_pagamento_pulito(scelta)
        combo_metodo.selection_clear()
        entry_imp_rapido.focus_set()
    ...
    def salva_rapido(e=None):
        ...
        _conto_per_voce_rap = _nome_c_rap if _nome_c_rap and _nome_c_rap != "(nessuno)" else ""
        _tags_rap = self._normalizza_tags(var_tag_rap.get())
        self.spese[data_sel].append(SpesaEntry.nuova(
            cat, desc_finale, imp, var_tipo.get(),
            conto=_conto_per_voce_rap, metodo_pagamento=_ultimo_metodo_rap["valore"],
            hashtag=_tags_rap
        ))
        if _tags_rap and hasattr(self, '_cache_tutti_tag'):
