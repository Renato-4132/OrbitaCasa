...
def apri_fondo_risparmio(self):
    ...
    def _build_proiezione():
        ...
        def _draw_barre(event=None):
            ...
            for tick in range(0, 6):
                v = max_val * tick / 5
                y = PT + ah - ah * tick / 5
                label_y = (f"{v/1000:.1f}".replace(".", ",") + "k" if v >= 1000 else f"{v:.0f}")
            ...
            for i, p in enumerate(proj):
                ...
                val_e   = (f"{p['e']/1000:.1f}".replace(".", ",") + "k" if p["e"] >= 1000 else f"{p['e']:.0f}")
                y_lbl_e = PT + ah - he - 8
                ...
                val_u   = (f"{p['u']/1000:.1f}".replace(".", ",") + "k" if p["u"] >= 1000 else f"{p['u']:.0f}")
                y_lbl_u = PT + ah - hu - 8
                ...
                segno   = "+" if p["r"] >= 0 else ""
                val_r   = (f"{p['r']/1000:.1f}".replace(".", ",") + "k" if abs(p["r"]) >= 1000 else f"{p['r']:.0f}")
                y_lbl_r = yr - 8
    ...
    def _build_mensile():
        ...
        obj_var = tk.StringVar(value=f"{fr_dati.get('obiettivo_annuale', 0):.2f}".replace(".", ","))
    ...
    def _build_trend():
        ...
        def _draw_spark(event=None):
            ...
            for i, (x, y) in enumerate(pts):
                ...
                cvs_sp.tag_bind(tag_pt, "<Enter>", lambda e, r=dati_12[i]:
                    self.show_tooltip(e, f"{MESI_ESTESI[r['m']-1]} {r['y']}: € {fmts(r['r'])}"))
                cvs_sp.tag_bind(tag_pt, "<Leave>", self.hide_tooltip)
    ...
    def _build_emergenza():
        ...
        tk.Label(frm_att_row, text="Fondo emergenza attuale (€):",
                 bg=self.COLOR_WIDGET_BG, fg=self.TEXT_COLOR,
                 font=("Arial", 9)).pack(side="left", padx=(0, 4))
        fondo_var = tk.StringVar(value=f"{fr_dati.get('fondo_attuale', 0):.2f}".replace(".", ","))
