...
def _veicoli_crea_tab(self, nb, v, db, win):
    tab = ttk.Frame(nb)
    img_tab_veicolo = self.icone_gui.get("veicoli")
    if img_tab_veicolo:
        nb.add(tab, image=img_tab_veicolo, text=f"  {v.get('nome','Veicolo')}  ", compound="left")
    else:
...
def _veicoli_estratto(self, db, nb, v_fmese=None, v_fanno=None):
    ...
    lines += [
        ...
        "  ─────────────────────",
        f"  Modello           : {v.get('modello', '─')}",
        f"  Targa             : {v.get('targa', '─')}",
        f"  Km Attuali        : {v.get('km_attuali', 0):,.0f}",
        f"  Km Iniziali       : {v.get('km_iniziale', 0):,.0f}",
        f"  Bollo             : {testo_bollo}",
        f"  Assicurazione     : {testo_ass}",
        f"  Revisione         : {testo_rev}",
        f"  Costo medio al km : {'€ ' + _fmt_it(costo_km, ',.3f') if costo_km else '—'}",
    ...
    for m in movimenti:
        ...
        km_txt = f"{float(m['km']):,.0f}" if str(m.get("km", "")).strip() not in ("", "0") else "—"
        lines.append(f"  {m.get('data', ''):<12} {cat_pulita:<22} {km_txt:>10} {_fmt_it(imp, '>11,.2f')}€   {m.get('descrizione', '')}")
...
def _veicoli_estratto_totale(self, db, v_fmese=None, v_fanno=None):
    ...
    for v in db["veicoli"]:
        ...
        lines.append(
            ...
            f"Costo/km: {'€ ' + _fmt_it(costo_km, ',.3f') if costo_km else '—'}"
        )
    lines += [
        "", linea_doppia,
    ...
    for m in tutti_i_movimenti:
        ...
        lines.append(f"  {m.get('data', ''):<12} {nome_v_pulito:<16} {cat_pula:<24} {_fmt_it(imp, '>11,.2f')}€   {m.get('descrizione', '')}")
