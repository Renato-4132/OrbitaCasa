...
def _genera_report_pdf_core(self, anno_da=None, anno_a=None, mese_filtro=0, sezioni=None):
    ...
    for giorno in giorni_ordinati:
        ...
        for entry in entries:
            if isinstance(entry, dict):
                cat  = entry.get("categoria", "Varie")
                desc = entry.get("descrizione", "")
                imp  = entry.get("importo", 0)
                tipo = entry.get("tipo", "Uscita")
            else:
                cat, desc, imp, tipo = entry[0], entry[1], entry[2], entry[3]
            try:
    ...
    def pagina_movimenti(titolo, sottotitolo, righe):
        ...
        def intestazione_tab(p, y):
            ...
            p.insert_text((MARG + 125, y + 11), "Descrizione", fontsize=7, color=C_WHITE, fontname="Helvetica-Bold")
            p.insert_text((MARG + 245, y + 11), "Conto",       fontsize=7, color=C_WHITE, fontname="Helvetica-Bold")
            p.insert_text((MARG + 305, y + 11), "Metodo",      fontsize=7, color=C_WHITE, fontname="Helvetica-Bold")
            p.insert_text((MARG + 355, y + 11), "Ora",         fontsize=7, color=C_WHITE, fontname="Helvetica-Bold")
            p.insert_text((MARG + 383, y + 11), "Tag",         fontsize=7, color=C_WHITE, fontname="Helvetica-Bold")
        ...
        for i, riga in enumerate(righe_ord):
            ...
            desc_corta  = desc[:22]  + "…" if len(desc)  > 22  else desc
            conto_corto = conto_r[:11] + "…" if len(conto_r) > 11 else conto_r
            metodo_corto = metodo_r[:9] + "…" if len(metodo_r) > 9 else metodo_r
            tag_corto   = tag_r[:11] + "…" if len(tag_r) > 11 else tag_r
            ...
            page.insert_text((MARG + 125, cy + 10), desc_corta,     fontsize=6.5, color=C_SUBTEXT,  fontname="Helvetica")
            page.insert_text((MARG + 245, cy + 10), conto_corto,    fontsize=6.5, color=c_conto,    fontname="Helvetica-Bold")
            page.insert_text((MARG + 305, cy + 10), metodo_corto,   fontsize=6.5, color=C_SUBTEXT,  fontname="Helvetica")
            page.insert_text((MARG + 355, cy + 10), ora_r,          fontsize=6.5, color=C_SUBTEXT,  fontname="Helvetica")
            page.insert_text((MARG + 383, cy + 10), tag_corto,      fontsize=6.5, color=C_SUBTEXT,  fontname="Helvetica")
            page.insert_text((W-MARG-60,  cy + 10), f"€{val:,.2f}", fontsize=7,   color=colore_val, fontname="Helvetica-Bold")
    ...
    cy = totali_row(page, cy, [
        ("ENTRATE", f"€ {tot_anno['in']:,.2f}",  C_GREEN),
        ("USCITE",  f"€ {tot_anno['out']:,.2f}", C_RED),
        ("SALDO",   f"€ {saldo_a:,.2f}",          c_saldo_a),
    ])
    labels_mesi = [
        f"{mesi_nomi[m]} ({'+' if (data_punti[f'curr_m_{m}_in']-data_punti[f'curr_m_{m}_out'])>=0 else ''}€{data_punti[f'curr_m_{m}_in']-data_punti[f'curr_m_{m}_out']:,.0f})"
    ...
    if sezioni.get("portafoglio", True) and conti_db:
        ...
        for c in conti_db:
            ...
            page.insert_text((W - MARG - 80, cy + 13), f"€ {saldo_c:,.2f}",
            ...
            page.insert_text((MARG + 18, cy + 33),
                             f"{_titolo_periodo}: +€{e_a:,.0f}  -€{u_a:,.0f}",
        ...
        page.insert_text((W - MARG - 85, cy + 16), f"€ {totale_portafoglio:,.2f}",
    ...
    if sezioni.get("storico", True) and anni_lista:
        ...
        cy = totali_row(page, cy, [
            ("Tot. Entrate", f"€ {tot_st['in']:,.2f}",  C_GREEN),
            ("Tot. Uscite",  f"€ {tot_st['out']:,.2f}", C_RED),
            ("Saldo Totale", f"€ {saldo_s:,.2f}",        c_saldo_s),
        ])
        labels_anni = [
            f"Anno {a} ({'+' if (data_punti[f'st_a_{a}_in']-data_punti[f'st_a_{a}_out'])>=0 else ''}€{data_punti[f'st_a_{a}_in']-data_punti[f'st_a_{a}_out']:,.0f})"
            for a in anni_lista
        ]
    ...
    if sezioni.get("mesi", True):
        for m in mesi_attivi:
            ...
            if tutte:
                ...
                pagina_movimenti(
                    _titolo_mese,
                    f"Entrate € {tot_m_in:,.2f}  |  Uscite € {tot_m_out:,.2f}  |  Saldo {s_segno}€ {saldo_m:,.2f}",
                    tutte
                )
    ...
    if sezioni.get("portafoglio", True):
        for nome_c in sorted(movimenti_per_conto.keys()):
            ...
            if righe:
                ...
                pagina_movimenti(
                    f"Conto: {nome_c}",
                    f"Entrate € {tot_c_in:,.2f}  |  Uscite € {tot_c_out:,.2f}  |  Saldo {s_segno}€ {saldo_c_r:,.2f}",
                    righe
                )
