...
def _genera_report_pdf_core(self, anno_da=None, anno_a=None, mese_filtro=0, sezioni=None):
    ...
    for giorno in giorni_ordinati:
        ...
        for entry in entries:
            ...
            conto    = _nome_conto(giorno, val, t_tipo)
            riga     = (data_str, cat, desc, val, t_tipo, conto)
            tot_st[suffix] += val
    ...
    def pagina_movimenti(titolo, sottotitolo, righe):
        ...
        def intestazione_tab(p, y):
            ...
            p.insert_text((MARG + 140, y + 11), "Descrizione", fontsize=7, color=C_WHITE, fontname="Helvetica-Bold")
            p.insert_text((MARG + 290, y + 11), "Conto",       fontsize=7, color=C_WHITE, fontname="Helvetica-Bold")
        ...
        for i, riga in enumerate(righe_ord):
            ...
            conto_corto = conto_r[:18] + "…" if len(conto_r) > 18 else conto_r
            ...
            page.insert_text((MARG + 55,  cy + 10), cat[:16],       fontsize=7,   color=C_TEXT,     fontname="Helvetica")
            page.insert_text((MARG + 140, cy + 10), desc_corta,     fontsize=7,   color=C_SUBTEXT,  fontname="Helvetica")
            page.insert_text((MARG + 290, cy + 10), conto_corto,    fontsize=6.5, color=c_conto,    fontname="Helvetica-Bold")
