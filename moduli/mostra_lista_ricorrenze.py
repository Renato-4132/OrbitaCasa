...
def mostra_lista_ricorrenze(self):
    ...
    columns = ("Categoria", "Conto", "Tag", "Tipo", "Importo", "Durata", "Saldate", "Data Inizio", "Data Fine", "Importo Totale", "Contabilizzato", "Residuo", "ID")
    tree = ttk.Treeview(main_frame, columns=columns, show="headings", height=12)
    tree.tag_configure("uscita", foreground="red")
    tree.tag_configure("entrata", foreground="green")
    larghezze = {"Categoria": 110, "Conto": 90, "Tag": 90, "Tipo": 50, "Importo": 60, "Durata": 30, "Saldate": 30, "Data Inizio": 60, "Data Fine": 60, "Importo Totale": 80, "Contabilizzato": 70, "Residuo": 60, "ID": 180}
    for col in columns:
    ...
    for i, (id_ricorrenza, dati) in enumerate(self.ricorrenze.items()):
        ...
        values = (cat, conto, tag_str, tipo_voce, f"{imp:,.2f} €", n_volte, volte_passate, data_inizio_str, data_fine, f"{importo_totale:,.2f} €", f"{importo_gia_pagato:,.2f} €", f"{importo_rimasto:,.2f} €", id_ricorrenza)
...
def on_ricorrenza_double_click(self, event):
    ...
    tree_movimenti = ttk.Treeview(
        popup_movimenti,
        columns=("data", "categoria", "conto", "importo", "saldato", "da_saldare"),
        show="headings"
    )
    tree_movimenti.pack(fill="both", expand=True, padx=10, pady=10)
    for col in ("data", "categoria", "conto", "importo", "saldato", "da_saldare"):
    ...
    for data_str in date_list_str:
        ...
        tree_movimenti.insert("", "end", values=(
            data_str, descrizione_ricorrenza, conto_ricorrenza, f"{importo_effettivo:,.2f} €", icona_p, icona_dp
        ), tags=(tag,))
