        ...
        if deleted_count > 0:
            ...
            try:
                with open(PORTAFOGLIO_BANCARIO, "r", encoding="utf-8") as _f:
                    _db_p = json.load(_f)
                for _data_s, _imp, _tipo in voci_cancellate:
                    _trasf = next((t for t in _db_p.get("trasferimenti",[])
                        if t.get("data") == _data_s
                        and abs(round(float(t.get("importo",0)),2) - _imp) < 0.01
                        and ((_tipo == "Uscita" and t.get("a") in ("__spese__","Contabilità"))
                          or (_tipo == "Entrata" and t.get("da") in ("__spese__","Contabilità")))
                    ), None)
                    if _trasf:
                        _conto_id = _trasf.get("da") if _tipo == "Uscita" else _trasf.get("a")
                        _segno = 1 if _tipo == "Uscita" else -1
                        for _c in _db_p.get("conti",[]):
                            if _c["id"] == _conto_id:
                                _c["saldo"] = round(float(_c.get("saldo",0)) + _segno * _imp, 2)
                                break
                        _db_p["trasferimenti"] = [t for t in _db_p["trasferimenti"] if t["id"] != _trasf["id"]]
                ...
                self.show_toast(f"Errore pulizia portafoglio: {_ex}")
...
def on_ricorrenza_double_click(self, event):
    ...
    tree_movimenti = ttk.Treeview(
        popup_movimenti,
        columns=("data", "categoria", "importo", "saldato", "da_saldare"),
        show="headings"
    )
    tree_movimenti.pack(fill="both", expand=True, padx=10, pady=10)
    for col in ("data", "categoria", "importo", "saldato", "da_saldare"):
        tree_movimenti.heading(
            col, 
            text=col.capitalize(), 
            command=lambda _col=col: self.treeview_sort_column(tree_movimenti, _col, False)
        )
        tree_movimenti.column(col, width=100 if col != "categoria" else 300, anchor="center")
    ...
    for data_str in date_list_str:
        ...
        tree_movimenti.insert("", "end", values=(
            data_str, descrizione_ricorrenza, f"{importo_effettivo:,.2f} €", icona_p, icona_dp
        ), tags=(tag,))
