...
def calcola_mancanti(self):
    ...
    def analizza():
        ...
        for cat, mesi_presenti in sorted(risultati.items()):
            ...
            valori_riga = [cat, f"€{spesa_totale:.2f}", f"€{media_spesa:.2f}", cadenza]
    ...
    def popola_form(event):
        ...
        try:
            media_str = str(valori[2]).replace("€", "").replace(",", ".").strip()
            self.imp_entry.delete(0, tk.END)
            self.imp_entry.insert(0, f"{float(media_str):.2f}")
    ...
    def analizza_annuali():
        ...
        for cat, mese, media, n_anni, presente in righe:
            ...
            tree_annuali.insert("", "end",
                                values=(cat, mesi_nomi[mese - 1], f"{media:.2f}", n_anni, simbolo),
                                tags=(tag,))
    ...
    # Analisi Ricorrenze da registrare nel tab check
