...
def export_stats(self):
    ...
    lines.append(f"{'Totale Uscite:':<{label_width}} {_fmt_it(tot_uscite, f'>{value_width},.2f')}")
    lines.append(f"{'Differenza:':<{label_width}} {_fmt_it(diff, f'>{value_width},.2f')} €")
    lines.append("═" * len(header))
...
def export_month_detail(self):
    ...
    if not tutti_movimenti:
        ...
        lines.append(f" • ENTRATE: {_fmt_it(tot_entrate, '>12,.2f')} €")
        lines.append(f" • USCITE:  {_fmt_it(tot_uscite, '>12,.2f')} €")
        lines.append(f" • SALDO:   {_fmt_it(saldo_finale, '>12,.2f')} €" + "\n")
        lines.append("═" * 96)
        ...
        if cat_spese:
            for cat in sorted(cat_spese, key=cat_spese.get, reverse=True):
                ...
                lines.append(f" {cat:<22.22}{conteggio:<8}{_fmt_it(importo_cat, '>12,.2f')} €{percentuale:>10.1f} %")
        else:
        ...
        for mov in tutti_movimenti:
            ...
            imp_str = f"{segno}{_fmt_it(imp_mov)}"
...
def export_anno_dettagliato(self):
    ...
    def format_row(label, values):
        ...
        return f"{label_fmt}{numeri}{_fmt_it(sum(values), '12,.2f')}"
    ...
    lines.append(f"{'SALDO FINALE:':<{label_width}}{_fmt_it(saldo, f'>{len(header) - label_width},.2f')} €")
    lines.append("═" * len(header))
    text = "\n".join(lines)
...
def export_storico_totale(self):
    ...
    def format_row(label, data_dict):
        ...
        for anno in anni_lista:
            val = data_dict[anno]
            row += _fmt_it(val, f'>{col_width},.2f')
            riga_sum += val
        row += _fmt_it(riga_sum, f'>{col_width+2},.2f')
        return row
    ...
    for anno in anni_lista:
        ...
        saldo_riga += _fmt_it(s, f'>{col_width},.2f')
        risparmio_totale += s
    saldo_riga += _fmt_it(risparmio_totale, f'>{col_width+2},.2f')
...
def show_export_preview(self, content, default_filename=None):
    if hasattr(self, '_export_preview_win') and self._export_preview_win and self._export_preview_win.winfo_exists():
        self._export_preview_win.lift()
        self._export_preview_win.focus_force()
        return
    import __main__ as _app
