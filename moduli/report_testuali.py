...
def export_stats(self):
    ...
    header = f"{'Categoria':<{label_width}} {'Descrizione':<{desc_width}} {'Importo (€)':>{value_width}}  {'Tipo':<{tipo_width}} {'Conto':<{conto_width}}"
    sep = "─" * len(header)
...
def export_month_detail(self):
    ...
    if not tutti_movimenti:
        ...
        lines.append("\n" + "═" * 96)
        lines.append(f"{'DETTAGLIO MOVIMENTI'.center(96)}")
        lines.append("═" * 106)
        lines.append(f"{'Data':<11}{'Categoria':<18}{'Descrizione':<38}{'Tipo':<10}{'Importo':>11}  {'Conto':<16}")
        lines.append("─" * 106)
        tutti_movimenti.sort(key=lambda x: x[0])
        for mov in tutti_movimenti:
            data_mov, cat_mov, desc_mov, tipo_mov, imp_mov, conto_mov = mov
            data_str = data_mov.strftime('%d/%m/%Y')
            segno = "+" if tipo_mov == "Entrata" else "-"
            imp_str = f"{segno}{imp_mov:.2f}"
            lines.append(f"{data_str:<11}{cat_mov:<18.18}{desc_mov:<38.38}{tipo_mov:<10}{imp_str:>11}  {conto_mov:<16.16}")
        lines.append("─" * 106)
