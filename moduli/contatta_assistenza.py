...
def apri_pannello_topic(self, topic):
    ...
    combo_cat.grid(row=2, column=0, sticky="w", padx=15, pady=(2, 0))
    tk.Label(popup, text="Descrizione:", bg=self.COLOR_BACKGROUND, fg=self.COLOR_HEADER,
             font=("Arial", 9, "bold")).grid(row=3, column=0, sticky="nw", padx=15, pady=(10, 0))
    frame_txt = tk.Frame(popup, bg=self.COLOR_HIGHLIGHT, padx=1, pady=1)
    frame_txt.grid(row=4, column=0, sticky="nsew", padx=15, pady=(2, 0))
    ...
    tk.Checkbutton(popup, text="Includi info sistema (OS, versione Python)",
                   ...
                   ).grid(row=6, column=0, sticky="w", padx=15, pady=(6, 0))
    ...
    tk.Checkbutton(popup, text="Allega Registro Anomalie",
                   ...
                   ).grid(row=7, column=0, sticky="w", padx=15, pady=(2, 0))
    if not log_exists:
        tk.Label(popup, text="  (nessun registro disponibile)", bg=self.COLOR_BACKGROUND,
                 fg=self.TEXT_COLOR, font=("Arial", 7, "italic")).grid(row=8, column=0, sticky="w", padx=15)
    btns_frame = tk.Frame(popup, bg=self.COLOR_BACKGROUND)
    btns_frame.grid(row=9, column=0, sticky="ew", padx=20, pady=12)
    ...
    def invia():
        ...
        if includi_info_var.get():
            corpo += (
                ...
                f"Processore: {platform.processor()}\n"
            )
        ...
        ok = self.invia_email_assistenza(
...
def invia_email_assistenza(self, titolo, messaggio, allegato=None, allegati_extra=None):
    try:
