...
def mostra_dare_avere(self):
    ...
    if self._gestore_partecipa() and _gestore not in _nomi_ico:
        _nomi_ico[_gestore] = f"PER· {_gestore}"
    nomi_parti = ["Tutti"] + sorted(_nomi_ico.values(), key=lambda x: x.split(" ", 1)[1].lower())
    ...
    def _aggiorna_tree_impl():
        ...
        if not righe:
            tree.insert("", "end",
                        values=("—", "Nessun dato",
                                "Nessuna spesa con tag PER·Nome o CNT·Nome trovata.",
                                "", "", "", "", "", ""),
                        tags=("sep",))
        if righe:
            tree.insert("", "end",
        ...
        if tot_dovuto:
            for nome in sorted(tot_dovuto.keys()):
                ...
                debito_res = tot_debit[nome]
                sal_tag = "verde" if debito_res < 0.01 else "rosso"
                lbl_riep.insert("end", f"  {nome:<18} ", "neutro")
                lbl_riep.insert("end", f"{_fmt_it(dov, '>11,.2f')} €  ", "giallo")
                lbl_riep.insert("end", f"{_fmt_it(ver, '>11,.2f')} €  ", "verde")
                lbl_riep.insert("end", f"{_fmt_it(debito_res, '>11,.2f')} €  ", sal_tag)
                sal_str = "Saldato" if debito_res < 0.01 else f"Deve {_fmt_it(debito_res)} EUR"
            ...
            lbl_riep.insert("end", f"{_fmt_it(tot_dov_all, '>11,.2f')} €  ", "giallo")
            ...
            if chi_deve_a_chi:
                ...
                for (debitore, creditore), importo in sorted(
                    ...
                    lbl_riep.insert("end", f"{_fmt_it(importo)} EUR\n", "rosso")
        else:
    ...
    def _build_export_text():
        ...
        for nome in sorted(tot_d2.keys()):
            ...
            sal_str = "SALDATO" if deb_r < 0.01 else f"DEVE {_fmt_it(deb_r)} EUR"
            riep_txt += (f"  {nome:<18} {_fmt_it(dov, '>12,.2f')} EUR  "
                         f"{_fmt_it(ver, '>12,.2f')} EUR  {_fmt_it(deb_r, '>15,.2f')} EUR   {sal_str}\n")
        riep_txt += "─" * 80 + "\n"
        tot_all = sum(tot_d2.values())
        ver_all = sum(tot_v2.values())
        ris_all = sum(tot_r2.values())
        riep_txt += (f"  {'TOTALE':<18} {_fmt_it(tot_all, '>12,.2f')} EUR  "
                     f"{_fmt_it(ver_all, '>12,.2f')} EUR  {_fmt_it(ris_all, '>15,.2f')} EUR\n")
        chi2 = {}
        for deb in _debiti_ref[0]:
        ...
        if chi2:
            ...
            for (debitore, creditore), importo in sorted(
                ...
                riep_txt += f"  {debitore} -> {creditore}: {_fmt_it(importo)} EUR\n"
