...
def apri_finestra_revisione_universale(self, movimenti):
    ...
    win.minsize(1450, 620)
    larghezza, altezza = 1450, 620
    ...
    def aggiungi_categoria():
        ...
        var_nuova_cat.set("")
        for _, _, _, _, _, _, combo, _ in righe:
    ...
    def applica_conto_a_tutti():
        ...
        for _, _, _, _, _, _, _, combo_conto in righe:
    ...
    def toggle_tutti():
        for _, var_check, _, _, _, _, _, _ in righe:
            var_check.set(seleziona_tutti_var.get())
    chk_header = ttk.Checkbutton(header, variable=seleziona_tutti_var, command=toggle_tutti)
    chk_header.grid(row=0, column=0, padx=2, sticky="w", ipadx=4)
    tk.Label(header, text="Data", width=13, anchor="w",
             background=self.COLOR_WIDGET_BG, foreground=self.TEXT_COLOR,
             font=("Arial", 9, "bold")).grid(row=0, column=1, padx=2, sticky="w")
    tk.Label(header, text="Descrizione", width=115, anchor="w",
             background=self.COLOR_WIDGET_BG, foreground=self.TEXT_COLOR,
             font=("Arial", 9, "bold")).grid(row=0, column=2, padx=2, sticky="w")
    tk.Label(header, text="Importo €", width=15, anchor="w",
             background=self.COLOR_WIDGET_BG, foreground=self.TEXT_COLOR,
             font=("Arial", 9, "bold")).grid(row=0, column=3, padx=2, sticky="w")
    tk.Label(header, text="Categoria", width=22, anchor="w",
             background=self.COLOR_WIDGET_BG, foreground=self.TEXT_COLOR,
             font=("Arial", 9, "bold")).grid(row=0, column=4, padx=2, sticky="w")
    tk.Label(header, text="Conto", width=16, anchor="w",
             background=self.COLOR_WIDGET_BG, foreground=self.TEXT_COLOR,
             font=("Arial", 9, "bold")).grid(row=0, column=5, padx=2, sticky="w")
    ttk.Separator(win, orient="horizontal").pack(fill="x", padx=10, pady=2)
    ...
    for mov in movimenti:
        ...
        ttk.Checkbutton(riga, variable=var_check).grid(row=0, column=0, padx=2, sticky="w")
        frame_data = tk.Frame(riga, background=self.COLOR_WIDGET_BG)
        frame_data.grid(row=0, column=1, padx=2, sticky="w")
        var_data = tk.StringVar(value=mov["data"].strftime("%d-%m-%Y"))
        entry_data = ttk.Entry(frame_data, textvariable=var_data, width=10)
        ...
        entry_desc = ttk.Entry(riga, textvariable=var_desc, width=100)
        entry_desc.grid(row=0, column=2, padx=2, sticky="w")
        var_imp = tk.StringVar(value=f"{mov['importo']:.2f}")
        entry_imp = ttk.Entry(riga, textvariable=var_imp, width=11)
        entry_imp.grid(row=0, column=3, padx=2, sticky="w")
        combo = ttk.Combobox(riga, values=self.categorie, state="readonly",
                             width=20, style="Border.TCombobox")
        ...
        righe.append((mov, var_check, var_data, var_desc, var_imp, entry_data, combo, combo_conto))
    ...
    def salva():
        ...
        for mov, var_check, var_data, var_desc, var_imp, entry_data, combo, combo_conto in righe:
            ...
            voce = (cat, desc, abs(importo), tipo)
            gia_presente = any(
