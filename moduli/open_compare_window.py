...
def open_compare_window(self):
    ...
    left_tree = ttk.Treeview(left_container, columns=("Categoria", "Data", "Entrata", "Uscita"), show="headings", height=14,
                             yscrollcommand=left_vsb.set, selectmode='browse')
    ...
    right_tree = ttk.Treeview(right_container, columns=("Categoria", "Data", "Entrata", "Uscita"), show="headings", height=14,
                              yscrollcommand=right_vsb.set, selectmode='browse')
    ...
    def update_tables():
        ...
        for cat, data, ent, usc in sorted(rows1, key=lambda x: x[0].lower()): 
            ...
            left_tree.insert("", "end", values=(cat, data, f"{_fmt_it(ent)} €", f"{_fmt_it(usc)} €"), tags=(tag,))
            tot_ent1, tot_usc1 = tot_ent1 + ent, tot_usc1 + usc
        diff1 = tot_ent1 - tot_usc1
        left_total_ent_lbl.config(text=f"{_fmt_it(tot_ent1)} €", fg="green")
        left_total_usc_lbl.config(text=f"{_fmt_it(tot_usc1)} €", fg="red")
        left_diff_val_lbl.config(
            text=f"{_fmt_it(diff1)} €",
        ...
        for cat, data, ent, usc in sorted(rows2, key=lambda x: x[0].lower()):
            tag = 'entrata' if ent > 0 else ('uscita' if usc > 0 else '')
            right_tree.insert("", "end", values=(cat, data, f"{_fmt_it(ent)} €", f"{_fmt_it(usc)} €"), tags=(tag,))
            tot_ent2, tot_usc2 = tot_ent2 + ent, tot_usc2 + usc
        diff2 = tot_ent2 - tot_usc2
        right_total_ent_lbl.config(text=f"{_fmt_it(tot_ent2)} €", fg="green")
        right_total_usc_lbl.config(text=f"{_fmt_it(tot_usc2)} €", fg="red")
        right_diff_val_lbl.config(
            text=f"{_fmt_it(diff2)} €",
    ...
    def do_preview_export():
        ...
        for cat in tutte_le_categorie:
            ...
            lines.append(f"{cat:<20.20} {'':<12} {_fmt_it(ent1, '12,.2f')} {_fmt_it(usc1, '12,.2f')}   {_fmt_it(ent2, '12,.2f')} {_fmt_it(usc2, '12,.2f')}   {_fmt_it(diff_ent, '12,.2f')} {_fmt_it(diff_usc, '12,.2f')}")
        ...
        lines.append("─" * 130)
        lines.append(f"{'TOTALI GENERALI':<33} {_fmt_it(tot_ent1, '12,.2f')} {_fmt_it(tot_usc1, '12,.2f')}   {_fmt_it(tot_ent2, '12,.2f')} {_fmt_it(tot_usc2, '12,.2f')}   {_fmt_it(diff_ent_tot, '12,.2f')} {_fmt_it(diff_usc_tot, '12,.2f')}")
        saldo1_str = f"{_fmt_it(diff1)} €"
        saldo2_str = f"{_fmt_it(diff2)} €"
