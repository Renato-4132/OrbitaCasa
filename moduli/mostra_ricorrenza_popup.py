...
def mostra_ricorrenza_popup(self):
    ...
    metodi_ric = ["", "📂 Movimenti", "───────────", "💰 Contanti", "🔄 RID/SDD", "🏦 Bonifico", "💎 C.Credito", "💜 C.Credito", "💳 C.Debito", "📶 Contactless", "📯 PayPal", "📮 Bollettino", "🏪 Prepagata", "🪙 Assegno", "🔘 Revolut", "🍎 Apple Pay", "🎯 Google Pay", "🏣 Postepay", "📲 Satispay", "🔀 Scalapay", "🛒 Amazon Pay"]
    ...
    def aggiorna_simbolo_metodo_ric(event=None):
            ...
            if metodo == "📂 Movimenti":
                    ...
                    self.ricorrenza_metodo.set("")
                    return
            simbolo = simboli_metodo.get(metodo, "")
            desc = self.ricorrenza_desc.get().strip()
            for s in simboli_metodo.values():
                    if desc.startswith(s):
                            desc = desc[len(s):].lstrip()
                            break
            
            self.ricorrenza_desc.set(f"{simbolo} {desc}".strip() if simbolo else desc)
