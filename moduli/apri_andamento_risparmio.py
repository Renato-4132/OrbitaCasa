...
def apri_andamento_risparmio(self):
    ...
    _tip_win = [None, None, None]  # [toplevel tooltip, id job "after", widget che l'ha schedulato]
    _chiusura_in_corso = [False]
    ...
    def _tooltip_show(event, txt):
        if _chiusura_in_corso[0]:
            return
        if _tip_win[0]:
            try: _tip_win[0].destroy()
            except: pass
            _tip_win[0] = None
        if _tip_win[1] and _tip_win[2] is not None:
            try: _tip_win[2].after_cancel(_tip_win[1])
            except: pass
            _tip_win[1] = None
        ...
        def _mostra():
            ...
            ttk.Label(tw, text=txt, style="Tooltip.TLabel",
    ...
    win.transient(self)
    win.bind("<Escape>", lambda e: _chiudi_finestra())
    ...
    _resize_job1 = [None]
    _last_wh1 = [None, None]
    def _on_configure1(event=None):
        if _chiusura_in_corso[0] or not _pronto[0]:
            return
        wh = (canvas1.winfo_width(), canvas1.winfo_height())
        if wh == tuple(_last_wh1):
            return
        _last_wh1[0], _last_wh1[1] = wh
        _debounce(_resize_job1, canvas1, _aggiorna_tab1)
    canvas1.bind("<Configure>", _on_configure1)
    ...
    def _disegna_linea(categorie, entrate_v, uscite_v, saldi_v, righe_ctx=None):
        ...
        for valori, colore, nome, tipo_click in serie:
            ...
            for i in range(n):
                ...
                tip = (f"{categorie[i]} ({nome})\n"
                       f"Entrate:  {formatta_italiano(entrate_v[i])} €\n"
                       f"Uscite:   {formatta_italiano(uscite_v[i])} €\n"
    ...
    _resize_job2 = [None]
    _last_wh2 = [None, None]
    def _on_configure2(event=None):
        if _chiusura_in_corso[0] or not _pronto[0]:
            return
        wh = (canvas2.winfo_width(), canvas2.winfo_height())
        if wh == tuple(_last_wh2):
            return
        _last_wh2[0], _last_wh2[1] = wh
        _debounce(_resize_job2, canvas2, _aggiorna_tab2)
    canvas2.bind("<Configure>", _on_configure2)
    ...
    def _chiudi_finestra():
        if _chiusura_in_corso[0]:
            return
        _chiusura_in_corso[0] = True
        for _job, _widget in ((_resize_job1, canvas1), (_resize_job2, canvas2)):
            if _job[0]:
                try: _widget.after_cancel(_job[0])
    ...
    btn_chiudi.bind("<Button-1>", lambda e: _chiudi_finestra())
    win.protocol("WM_DELETE_WINDOW", _chiudi_finestra)
