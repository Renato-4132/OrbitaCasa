...
def mostra_log_importazioni(self):
    ...
    for i, col in enumerate(colonne):
        tree.heading(col, text=col, anchor="w", command=lambda c=col: self.sort_by(c))
        tree.column(col, width=larghezze[i], minwidth=larghezze[i], stretch=(i == len(colonne) - 1), anchor="w")
