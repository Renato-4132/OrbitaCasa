...
def on_stats_table_double_click(self, event):
    ...
    if mode == "mese":
        ...
        for d, sp in self.spese.items():
            if d.month == mese and d.year == anno:
                for idx, entry in enumerate(sp):
                    cat, desc, imp, tipo = entry[:4]
                    if cat.strip() == categoria: 
                        spese_categoria.append((d, desc, imp, tipo, entry, idx))
    ...
    for d, desc, imp, tipo, entry, idx in sorted(spese_categoria, key=lambda x: x[0], reverse=True):
        imp_formattato_it = f"{imp:,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.')
        tag_name = f"row_{d.strftime('%Y%m%d%H%M%S')}_{len(tree.get_children(''))}"
        if d:
            ordinale = self._ordine_in_gruppo_chiave(d, idx) if hasattr(self, "_ordine_in_gruppo_chiave") else 0
            nome_conto_tr = campo(entry, "conto", "") or self._trova_conto_da_portafoglio(d, imp, tipo, ordinale)
        else:
            nome_conto_tr = ""
