...
def on_stats_table_double_click(self, event):
    ...
    if testo_ricerca and mode == "giorno":
        try:
            def pulisci_importo_ricerca(testo):
                if not testo:
                    return 0.0
    ...
    ref_periodo = self.stats_refdate
    if testo_ricerca:
        try:
            from datetime import datetime as _dt3
            ref_periodo = _dt3.strptime(str(values[0]).strip(), "%d-%m-%Y").date()
        except Exception:
            ref_periodo = self.stats_refdate
            ...
            if d.month == mese and d.year == anno:
                for entry in sp:
                    ...
                    if cat.strip() == categoria: 
                        conto_v = campo(entry, "conto", "")
                        metodo_v = campo(entry, "metodo_pagamento", "")
                        tag_v = " ".join(campo(entry, "hashtag", []) or [])
    ...
    columns = ("Data", "Descrizione", "Importo", "Tipo", "Conto", "Metodo", "Tag")
    tree = ttk.Treeview(
        tree_frame, 
        columns=columns, 
        show="headings", 
    ...
    for col, w in zip(columns, (90, 210, 90, 80, 120, 100, 140)):
