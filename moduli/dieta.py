...
def apri_dieta(self):
    ...
    def _apri_form_custom(prefill=None):
        ...
        for i, (lbl, key, tipo, default) in enumerate(campi):
            ...
            tk.Label(fc, text=f"{lbl}:", bg=self.COLOR_WIDGET_BG,
                     fg=self.COLOR_TEXT, font=("Arial", 9)).grid(
    ...
    def _elimina_custom():
        ...
        ttk.Label(ed, text="Seleziona le voci da eliminare:",
                  font=("Arial", 9, "bold"),
                  background=self.COLOR_WIDGET_BG,
                  foreground=self.COLOR_TEXT).pack(padx=14, pady=(10,4), anchor="w")
    ...
    def _confronta_settimane():
        ...
        tv = ttk.Treeview(fr_tv, columns=colonne, show="headings",
            ...
            try:
                ...
                if ped_dati:
                    ...
                    media30 = [v for (ds, _), v in zip(voci_ordinate, vals_p)
                               if (oggi_d - datetime.datetime.strptime(ds, "%d-%m-%Y").date()).days < 30]
                    riepilogo = (
    ...
    def _apri_composizione(s_idx, g_idx, pasto_key):
        ...
        tk.Label(left, text="Alimento:", font=("Arial",8,"bold"),
                 bg=self.COLOR_WIDGET_BG, fg=self.COLOR_TEXT).pack(anchor="w")
        ...
        tk.Label(gr_fr, text="Grammi:", bg=self.COLOR_WIDGET_BG,
                 fg=self.COLOR_TEXT, font=("Arial",9)).pack(side="left")
        ...
        for k, txt in [("kcal","Kcal: —"),("prot","Prot.: —"),("grassi","Gras.: —"),
            ...
            l = tk.Label(tot_fr, text=txt,
                         ...
                         fg=self.COLOR_HIGHLIGHT if bold else self.COLOR_TEXT)
    ...
    for s_idx, sett in enumerate(piano_dati):
        ...
        tree = ttk.Treeview(frame, columns=COL_PIANO, show="headings",
                            yscrollcommand=vsb.set, style="Treeview")
    ...
    tk.Label(fr_inp, text="Data:", bg=self.COLOR_WIDGET_BG,
             fg=self.COLOR_TEXT, font=("Arial", 9)).grid(row=0, column=0, sticky="e", pady=3)
    ...
    tk.Label(fr_inp, text="Peso (kg):", bg=self.COLOR_WIDGET_BG,
             fg=self.COLOR_TEXT, font=("Arial", 9)).grid(row=1, column=0, sticky="e", pady=3)
    peso_v = tk.StringVar()
    ...
    tk.Label(fr_inp, text="Obiettivo (kg):", bg=self.COLOR_WIDGET_BG,
             fg=self.COLOR_TEXT, font=("Arial", 9)).grid(row=2, column=0, sticky="e", pady=3)
    ...
    tk.Label(fr_inp, text="Altezza (cm):", bg=self.COLOR_WIDGET_BG,
             fg=self.COLOR_TEXT, font=("Arial", 9)).grid(row=3, column=0, sticky="e", pady=3)
    ...
    tv_peso = ttk.Treeview(fr_tv_p,
                           ...
                           yscrollcommand=vsb_p.set, height=10)
    ...
    def _disegna_grafico(*_):
        ...
        if not peso_storico or len(peso_storico) < 1:
            cvs_peso.create_text(W//2, H//2,
                ...
                fill=self.COLOR_TEXT, font=("Arial", 10))
            return
        pesi  = [r["peso"] for r in peso_storico]
        n     = len(pesi)
        p_min = min(pesi)
        ...
        for i, (x, y) in enumerate(pts):
            ...
            cvs_peso.create_text(x, y - 12,
                                  text=f"{pesi[i]}",
                                  fill=self.COLOR_TEXT, font=("Arial", 7))
        step_x = max(1, n // 8)
    ...
    for i, label in enumerate(["Sesso", "Peso (kg)", "Altezza (cm)", "Età"]):
            tk.Label(fr_input, text=label+":", bg=self.COLOR_WIDGET_BG, fg=self.COLOR_TEXT).grid(row=0, column=i*2, padx=5)
            if label == "Sesso":
    ...
    tk.Label(fr_ins, text="Data:", bg=self.COLOR_WIDGET_BG,
             fg=self.COLOR_TEXT, font=("Arial", 9)).grid(row=0, column=0, sticky="w", pady=3)
    ...
    tk.Label(fr_ins, text="Passi:", bg=self.COLOR_WIDGET_BG,
             fg=self.COLOR_TEXT, font=("Arial", 9)).grid(row=1, column=0, sticky="w", pady=3)
    ...
    tk.Label(fr_ins, text="Obiettivo:", bg=self.COLOR_WIDGET_BG,
             fg=self.COLOR_TEXT, font=("Arial", 9)).grid(row=2, column=0, sticky="w", pady=3)
    ...
    tk.Label(fr_ins, text="Note:", bg=self.COLOR_WIDGET_BG,
             fg=self.COLOR_TEXT, font=("Arial", 9)).grid(row=3, column=0, sticky="w", pady=3)
    ...
    for i, (chiave, testo) in enumerate([
        ...
        tk.Label(fr_stats, text=testo, bg=self.COLOR_WIDGET_BG,
                 fg=self.COLOR_TEXT, font=("Arial", 8), anchor="w").grid(
    ...
    tk.Label(fr_filt, text="Mese:", bg=self.COLOR_WIDGET_BG,
             fg=self.COLOR_TEXT, font=("Arial", 8)).pack(side="left")
    ...
    def _ped_aggiorna_stats(righe):
        ...
        if oggi_rec:
            ...
            ped_stat_labels["oggi"].config(text="—", fg=self.COLOR_TEXT)
            ped_stat_labels["ob_pct"].config(text="—", fg=self.COLOR_TEXT)
        ...
        if tutti:
            ...
            ultimi7  = [t[1] for t in tutti
                        if (oggi_d - datetime.datetime.strptime(t[0],"%d-%m-%Y").date()).days < 7]
            ultimi30 = [t[1] for t in tutti
                        if (oggi_d - datetime.datetime.strptime(t[0],"%d-%m-%Y").date()).days < 30]
            ...
            for k in ("record","media7","media30","tot_mese","giorni_ok"):
                ped_stat_labels[k].config(text="—", fg=self.COLOR_TEXT)
    def _ped_disegna_grafico(righe):
        ...
        for k in range(5):
            ...
            ped_canvas.create_text(pad_l - 4, gy,
                ...
                anchor="e", font=("Arial", 7), fill=self.COLOR_TEXT)
        ...
        for i, r in enumerate(righe_graf):
            ...
            if n <= 10 or i % max(1, n // 10) == 0:
                ped_canvas.create_text(cx, H - pad_b + 4, text=r[0][:5],
                                       anchor="n", font=("Arial", 7), fill=self.COLOR_TEXT)
        ...
        ped_canvas.create_text(leg_x+14, leg_y+4, text="Obiettivo raggiunto",
                               anchor="w", font=("Arial", 7), fill=self.COLOR_TEXT)
    ...
    lbl_profilo_info = tk.Label(fr_sel, text="", bg=self.COLOR_WIDGET_BG,
                         fg=self.COLOR_TEXT, font=("Arial", 8),
