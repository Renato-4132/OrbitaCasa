...
_LEGENDA_TAG_TESTO = (
    "Legenda tag: [Q] Quota Personale (⚖️)   "
    "[C] Casa/Gruppo (🏠)   "
    "[P] Partecipante (👤)   "
    "[R] Ricorrente (♻️)   "
    "[A] Allegato (📎)"
...
def _testo_sicuro_monospace(testo):
    for emoji, tag in _TAG_EMOJI_DESCRIZIONE.items():
        testo = testo.replace(emoji, tag)
    return testo

def _crea_pdf_da_testo_monospace(testo, titolo="Esportazione"):
    W, H = 842, 595 
    MARG_X            = 30
    MARG_Y_TOP         = 44
    MARG_Y_BOTTOM      = 28
    ...
    righe              = testo.split("\n")
    righe_per_pagina   = max(1, int((H - MARG_Y_TOP - MARG_Y_BOTTOM) / LINE_H))
    pagine             = [righe[i:i + righe_per_pagina] for i in range(0, len(righe), righe_per_pagina)] or [[]]
    doc = fitz.open()
    for idx, blocco in enumerate(pagine):
        page = doc.new_page(width=W, height=H)
        page.insert_text((MARG_X, 24), titolo, fontsize=10, fontname="Helvetica-Bold", color=(0.17, 0.24, 0.31))
        testo_pag = f"Pag. {idx + 1}/{len(pagine)}"
        page.insert_text((W - MARG_X - fitz.get_text_length(testo_pag, fontname="Helvetica", fontsize=8), 24),
                          testo_pag, fontsize=8, fontname="Helvetica", color=(0.5, 0.55, 0.6))
        y = MARG_Y_TOP
        for riga in blocco:
            page.insert_text((MARG_X, y), riga, fontsize=FONT_SIZE, fontname="Courier", color=(0, 0, 0))
            y += LINE_H
    return doc
...
def cerca_operazioni(self):
    ...
    btn_cal_da = ttk.Label(frame_superiore, image=self.icone_gui.get("calendario"),
    ...
    btn_cal_a = ttk.Label(frame_superiore, image=self.icone_gui.get("calendario"),
                          cursor="hand2", background=self.COLOR_WIDGET_BG)
    ...
    def esegui_ricerca(event=None):
        ...
        for data_key in sorted(self.spese.keys(), reverse=True):
            ...
            for voce in self.spese[data_key]:
                ...
                owner = next((p["nome"] for p in self.nomi_partecipanti
    ...
    def esporta_risultato():
        ...
        header    = (f"{'Data':<{W_DATA}} │ {'Categoria':<{W_CAT}} │ {'Descrizione':<{W_DESC}} │ {'Tipo':<{W_TIPO}} │ "
                     f"{'Importo (€)':>{W_IMP}} │ {'Conto':<{W_CNT}} │ {'Ora':<{W_ORA}} │ {'Hashtag':<{W_TAG}} │ {'Metodo':<{W_MET}}\n")
        ...
        contenuto_preview += f" {_LEGENDA_TAG_TESTO}\n"
        ...
        for data, cat, desc, tipo, importo_float, conto, ora, hashtag, metodo in dati_tree:
            desc_sicura   = _testo_sicuro_monospace(desc)
            desc_troncata = (desc_sicura[:W_DESC-3] + '...') if len(desc_sicura) > W_DESC else desc_sicura
        ...
        def _salva_come_pdf(content_text, preview_popup):
            ...
            file = filedialog.asksaveasfilename(
                ...
                initialdir=EXPORT_FILES,
                initialfile=nome_file,
                title="Salva risultati ricerca (PDF)",
                confirmoverwrite=False,
                parent=finestra
            )
            if file:
                if os.path.exists(file):
                    if not self.show_custom_askyesno("Sovrascrivere file?",
                        ...
                        return
                try:
                    doc_pdf = _crea_pdf_da_testo_monospace(
                        content_text,
                        titolo=f"Risultati Ricerca - {datetime.date.today():%d/%m/%Y}"
                    )
                    doc_pdf.save(file)
                    doc_pdf.close()
                    self.show_custom_warning("Esportazione completata", f"✓ Risultati salvati:\n{file}")
                except Exception as e:
                    self.show_custom_warning("Errore", f"❌ Salvataggio PDF fallito:\n{e}")
        ...
        frame_testo = tk.Frame(preview_popup)
        frame_testo.pack(fill='both', expand=True, padx=10, pady=10)
        scroll_preview = ttk.Scrollbar(frame_testo, orient="vertical", style="Vertical.TScrollbar")
        scroll_preview.pack(side='right', fill='y')
        text_area = tk.Text(frame_testo, wrap='word', font=('Courier', 10), padx=10, pady=10,
                             yscrollcommand=scroll_preview.set)
        text_area.insert('1.0', contenuto_preview)
        text_area.config(state='disabled')
        text_area.pack(side='left', fill='both', expand=True)
        scroll_preview.config(command=text_area.yview)
        text_area.bind("<MouseWheel>", lambda e: text_area.yview_scroll(int(-1 * (e.delta / 120)), "units"))
        text_area.bind("<Button-4>",   lambda e: text_area.yview_scroll(-1, "units"))
        text_area.bind("<Button-5>",   lambda e: text_area.yview_scroll(1, "units"))
        ...
        img_esporta_pdf = self.icone_gui.get("pdf") or self.icone_gui.get("salva")
