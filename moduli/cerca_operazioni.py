...
def cerca_operazioni(self):
    ...
    columns = ("Data", "Categoria", "Descrizione", "Tipo", "Importo", "Conto")
    ...
    def esporta_risultato():
        ...
        for item in tree.get_children():
            ...
            if values:
                data, cat, desc, tipo, importo_str, conto = values[0], values[1], values[2], values[3], values[4], values[5] if len(values) > 5 else ""
                importo_float = converti_importo(importo_str)
                dati_tree.append((data, cat, desc, tipo, importo_float, conto))
                if tipo == "Entrata":
        ...
        if not dati_tree:
            ...
            return
        W_DATA, W_CAT, W_DESC, W_TIPO, W_IMP, W_CNT = 10, 20, 26, 8, 15, 14
        header    = f"{'Data':<{W_DATA}} │ {'Categoria':<{W_CAT}} │ {'Descrizione':<{W_DESC}} │ {'Tipo':<{W_TIPO}} │ {'Importo (€)':>{W_IMP}} │ {'Conto':<{W_CNT}}\n"
        separator = f"{'─'*W_DATA}─┼─{'─'*W_CAT}─┼─{'─'*W_DESC}─┼─{'─'*W_TIPO}─┼─{'─'*W_IMP}─┼─{'─'*W_CNT}\n"
        ...
        for data, cat, desc, tipo, importo_float, conto in dati_tree:
