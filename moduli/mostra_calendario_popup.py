        ...
        if hasattr(self, 'spese') and self.spese:
            for d_str, entries in self.spese.items():
                ...
                for e in entries:
                    ...
                    txt += f"{segno} {cat}: {_app._fmt_it(float(imp))}\n"
                    if segno == "+": e_sum += 1
                    else: u_sum += 1
            ...
            try:
                ...
                if widget and "label" in str(widget) and widget.cget("text").isdigit():
                    ...
                    if righe_raw or data_h == datetime.date.today():
                        ...
                        for blocco in righe_raw:
                            for r in blocco.split('\n'):
                                ...
                                try:
                                    ...
                                    valore = _app._parse_it_float(parti[1].strip())
                                    r_data = (desc, _app._fmt_it(valore))
                                    if "+" in r:
                        ...
                        if entrate:
                            crea_riga(main_f, "▲ SALDO (+):", _app._fmt_it(tot_e), self.COLOR_GREEN_SMOOTH, True)
                        ...
                        if uscite:
                            ...
                            crea_riga(main_f, "▼ SALDO (-):", _app._fmt_it(tot_u), self.COLOR_RED_SMOOTH, True)
