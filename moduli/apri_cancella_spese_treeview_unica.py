...
def apri_cancella_spese_treeview_unica(self):
        ...
        if os.path.exists(PORTAFOGLIO_BANCARIO):
            try:
                with open(PORTAFOGLIO_BANCARIO, "r", encoding="utf-8") as _f:
                    _db_p = json.load(_f)
                for giorno_obj, voci in self.spese_cancellate_tmp.items():
                    for voce in voci:
                        try:
                            imp = float(voce[2])
                            tipo = voce[3]
                            data_s = giorno_obj.strftime("%d-%m-%Y")
                            nuovi = []
                                ...
                                if t.get("data") == data_s and abs(float(t.get("importo",0)) - imp) < 0.01:
                                    if tipo == "Uscita" and t.get("a") in ("__spese__","Contabilità"):
                                        for c in _db_p["conti"]:
                                            if c["id"] == t.get("da"):
                                                c["saldo"] = round(float(c.get("saldo",0)) + imp, 2)
                                        continue
            ...
            for indice, voce in enumerate(lista_voci): 
                try:
                    ...
                    if matches and filtri.get("conto", "—") not in ("", "—"):
                        nome_conto_sp = self._trova_conto_da_portafoglio(d, float(importo), tipo)
                    ...
                    if matches:
                        ...
                        nome_conto_sp = self._trova_conto_da_portafoglio(d, float(importo), tipo) if d else ""
                        self.spese_treeview.insert(
                            "", "end", iid=iid,
                            text=checkbox_text,
                            values=(giorno_visualizzato, categoria, descrizione, tipo, importo_formattato, nome_conto_sp),
                            tags=tags
                        )
                        items_inserted += 1
                except Exception:
    ...
    def apri_filtri_avanzati():
        ...
        crea_riga("Pagamento:", icona_var, ["—", "💰 Contanti", "🔄 RID/SDD", "🏦 Bonifico", "💎 C.Credito", "💜 C.Credito", "💳 C.Debito", "📶 Contactless", "📯 PayPal", "📮 Bollettino", "🏪 Prepagata", "🪙 Assegno", "🔘 Revolut", "🍎 Apple Pay", "🎯 Google Pay", "🏣 Postepay", "📲 Satispay", "🔀 Scalapay", "🛒 Amazon Pay"])
    ...
    self.spese_treeview = ttk.Treeview(
        tree_frame,
        columns=("Giorno", "Categoria", "Descrizione", "Tipo", "Importo", "Conto"), 
        show=("tree", "headings"), 
