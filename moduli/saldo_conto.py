...
def open_saldo_conto(self):
        ...
        def build_riepilogo():
            ...
            if not conti:
                ...
                for c in conti:
                    ...
                    tk.Label(card, text=f"€ {_fmt_it(saldo_c)}",
            ...
            def _riga(etichetta, valore, colore_val=None, bold=False):
                ...
                tk.Label(r, text=f"€ {_fmt_it(valore, '>12,.2f')}", font=font_val,
            ...
            def anteprima_export_riepilogo():
                ...
                for c in conti_:
                    ...
                    contenuto += (f"{c.get('nome',''):<22} {c.get('tipo','').capitalize():<14} "
                                  f"€ {_fmt_it(self._saldo_effettivo(c, db_now), '>12,.2f')}  {princ}\n")
                contenuto += sep2
                contenuto += f"{'Totale conti:':<37} € {_fmt_it(totale_conti, '>12,.2f')}\n"
                contenuto += sep
                ...
                contenuto += f"{'Totale conti registrati:':<36}  € {_fmt_it(totale_conti, '>11,.2f')}\n"
                d_col = "+" if delta_mese >= 0 else ""
                contenuto += f"{'Delta spese mese (' + oggi.strftime('%m/%Y') + '):':<37} {d_col}€ {_fmt_it(delta_mese, '>11,.2f')}\n"
                contenuto += sep2
                s_col = "+" if saldo_netto >= 0 else ""
                contenuto += f"{'SALDO NETTO SPENDIBILE:':<37} {s_col}€ {_fmt_it(saldo_netto, '>11,.2f')}\n"
                contenuto += sep
        ...
        def build_conti():
                ...
                for c in db_now.get("conti", []):
                    ...
                    iid = tree.insert("", "end", iid=c["id"],
                                      values=(c.get("nome",""), c.get("tipo",""),
                                              f"{_fmt_it(saldo_c)}", princ, c.get("note","")))
                    colore = TIPO_COLORI.get(c.get("tipo","altro"), "#A78BFA")
            ...
            def popola_form(conto):
                ...
                v_saldo.set(f"{float(conto.get('saldo', 0.0)):.2f}".replace(".", ","))
        ...
        def build_movimenti():
            ...
            def aggiorna_tree(*_):
                ...
                lbl_riepilogo.config(
                    text=f"  Entrate: € {_fmt_it(tot_e)}   Uscite: € {_fmt_it(tot_u)}   "
                         f"Saldo: € {_fmt_it(saldo_m, '+,.2f')}   ({n_tot} movimenti)",
                    fg=col_s)
            ...
            def on_click_dx_mov(e):
                ...
                try:
                    imp   = float(str(r[3]).replace(".","").replace(",","."))
                    cat   = str(r[1])
                    desc  = str(r[2])
                    tipo  = str(r[4])
                except Exception:
                ...
                try:
                    ...
                    self.imp_entry.insert(0, f"{imp:.2f}".replace(".", ","))
            ...
            def aggancia_a_conto():
                ...
                for iid in sel:
                    ...
                    try:
                        data_v = str(vals[0])
                        imp    = float(str(vals[3]).replace(".","").replace(",", "."))
                        tipo   = str(vals[4])
            ...
            def rimuovi_agganciato():
                ...
                for iid in sel_agg:
                    ...
                    try:
                        data_v = str(vals[0])
                        imp    = round(float(str(vals[3]).replace(".","").replace(",", ".")), 2)
                        tipo   = str(vals[4])
                        d_obj  = datetime.datetime.strptime(data_v, "%d-%m-%Y").date()
                    except Exception:
                        continue
        ...
        def build_storico():
            ...
            def disegna(event=None):
                ...
                for k in range(6):
                    ...
                    c.create_text(pad_l-4, yp, text=f"€{_fmt_it(yv, ',.0f')}",
                ...
                def serie(campo, colore, raggio=3):
                    ...
                    for i, (px, py) in enumerate(pts):
                        ...
                        tip = (f"{titolo_tip}\n"
                               f"Entrate:  €{_fmt_it(d['ent'])}\n"
                               f"Uscite:   €{_fmt_it(d['usc'])}\n"
                               f"Saldo:    €{_fmt_it(d['saldo'])}")
                        c.tag_bind(ov, "<Enter>",
