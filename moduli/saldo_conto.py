...
def open_saldo_conto(self):
        ...
        def build_trasferimenti():
            ...
            def reset_t():
                ...
                sel_id[0] = None
            def _applica_saldi(db_r, da_id, a_id, imp, segno=1):
                for c in db_r["conti"]:
                    if c["id"] == da_id:
                        c["saldo"] = float(c.get("saldo", 0)) - imp * segno
                    if c["id"] == a_id:
                        c["saldo"] = float(c.get("saldo", 0)) + imp * segno
            def salva_trasf():
                if not nomi_conti:
                ...
                if sel_id[0]:
                    ...
                    if vecchio:
                        _applica_saldi(db_r, vecchio["da"], vecchio["a"],
                                       float(vecchio["importo"]), segno=-1)
                        vecchio["data"]    = v_data.get()
                        vecchio["da"]      = da_id
                        vecchio["a"]       = a_id
                        vecchio["importo"] = imp
                        vecchio["note"]    = v_note.get().strip()
                    _applica_saldi(db_r, da_id, a_id, imp, segno=1)
                else:
                    _applica_saldi(db_r, da_id, a_id, imp, segno=1)
        ...
        def build_movimenti():
            ...
            cols = ("data","categoria","descrizione","importo","tipo","conto")
                ...
                for d in sorted(self.spese.keys(), reverse=True):
                    ...
                    for v in self.spese[d]:
                        ...
                        if d > oggi:
                            ...
                            tag = "uscita"
                        tree.insert("", "end",
                                    values=(d.strftime("%d-%m-%Y"), cat, desc, f"{imp:,.2f}", tipo, nome_conto_sp),
                                    tags=(tag,))
            ...
            def aggancia_a_conto():
                ...
                if not nome_conto:
                    ...
                    return
                db_r  = carica_db()
                conti = db_r.get("conti", [])
                conto = next((c for c in conti if c.get("nome") == nome_conto), None)
                if not conto:
                    self.show_toast("Conto non trovato.")
                    return
                agganciati = 0
                for iid in sel:
                    ...
                    try:
                        ...
                        continue
                    delta = imp if tipo == "Entrata" else -imp
                    conto["saldo"] = float(conto.get("saldo", 0)) + delta
                    if tipo == "Entrata":
                        da_id, a_id = "__spese__", conto["id"]
                    else:
                        da_id, a_id = conto["id"], "__spese__"
                    db_r["trasferimenti"].append({
                        "id":      nuovo_id("t", db_r["trasferimenti"]),
                        "data":    data_v,
                        "da":      da_id,
            ...
            def rimuovi_agganciato():
                ...
                if not sel_agg:
                    self.show_toast("Seleziona un movimento agganciato.")
                    return
                if not self.show_custom_askyesno("Rimuovi", "Rimuovere i movimenti agganciati selezionati e ripristinare il saldo?"):
                    return
                db_r = carica_db()
                rimossi = 0
                for iid in sel_agg:
                    ...
                    try:
                        ...
                        continue
                    trasf = next((
                        t for t in db_r.get("trasferimenti", [])
                        if t.get("data") == data_v
                        and abs(round(float(t.get("importo",0)),2) - imp) < 0.01
                        and ((tipo == "Uscita" and t.get("a") in ("__spese__","Contabilità"))
                          or (tipo == "Entrata" and t.get("da") in ("__spese__","Contabilità")))
                    ), None)
                    if not trasf:
                        continue
                    if trasf.get("da") in ("__spese__","Contabilità"):
                        conto_id = trasf.get("a")
                        delta = -imp
                    else:
                        conto_id = trasf.get("da")
                        delta = imp
                    for c in db_r.get("conti", []):
                        if c["id"] == conto_id:
                            c["saldo"] = float(c.get("saldo", 0)) + delta
                            break
                    db_r["trasferimenti"] = [t for t in db_r["trasferimenti"] if t["id"] != trasf["id"]]
                    rimossi += 1
                salva_db(db_r)
