...
def _migra_valore(obj, mappa):
    if isinstance(obj, dict):
        ...
        for k, v in obj.items():
            ...
            conteggio += c
            if nuova_chiave in nuovo and nuova_chiave != k:
                print(f"[{time.strftime('%H:%M:%S')}] Migrazione emoji: COLLISIONE su chiave "
                      f"'{nuova_chiave}' (da '{k}') — voce originale conservata, questa scartata per evitare perdita dati.")
                continue
