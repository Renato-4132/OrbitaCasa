...
def mostra_spese_simili(self):
    ...
    self.popup_simili = popup
    popup.title(f"Movimenti simili a €{_fmt_it(target)}")
    ...
    label_range = ttk.Label(
        popup,
        text=f"Movimenti compresi tra €{_fmt_it(limite_basso)} e €{_fmt_it(limite_alto)}",
        style="White.TLabel"
    )
    ...
    def aggiorna_auto(*args):
        ...
        label_range.config(text=f"Movimenti compresi tra €{_fmt_it(limite_basso)} e €{_fmt_it(limite_alto)}")
        nuove_voci = [
            (d, voce)
            for d, lista in self.spese.items()
            for voce in lista
