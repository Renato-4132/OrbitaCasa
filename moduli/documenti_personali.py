                ...
                def _analizza_dp():
                    try:
                        ...
                        def _aggiorna_gui_dp():
                            ...
                            self.show_toast(f"ALL· {os.path.basename(pdf_path)} — campi compilati, verifica e archivia")
...
def backup_documenti_personali(self):
    ...
    try:
        ...
        profili_file = os.path.join(DOC_PERS_DIR, "profili.json")
        if files:
            ultimo_backup_mtime = os.path.getmtime(files[-1])
            ultima_modifica = 0.0
            if os.path.exists(DOC_PERS_DIR):
                for root, _, fnames in os.walk(DOC_PERS_DIR):
                    for fn in fnames:
                        m = os.path.getmtime(os.path.join(root, fn))
                        if m > ultima_modifica:
                            ultima_modifica = m
            if os.path.exists(profili_file):
                ultima_modifica = max(ultima_modifica, os.path.getmtime(profili_file))
            if ultima_modifica <= ultimo_backup_mtime:
                print(f"[{datetime.now().strftime('%H:%M:%S')}] Backup Documenti Personali: nessuna modifica, salto.")
                return files[-1]
        for f in files[:-2]:
            os.remove(f)
        ts  = datetime.now().strftime("%d%m%Y_%H%M")
        arc = os.path.join(bk_dir, f"Archivio_Doc_Personali_{ts}.zip")
        with zipfile.ZipFile(arc, "w", zipfile.ZIP_DEFLATED, compresslevel=1) as zf:
            if os.path.exists(DOC_PERS_DIR):
                for root, _, fnames in os.walk(DOC_PERS_DIR):
                    for fn in fnames:
...
def mostra_help_documenti_personali(self):
    ...
    sezione("• Compila Data, Categoria, Descrizione (max 30 car.) e Note (max 30 car.), poi clicca Archivia PDF.\n"
