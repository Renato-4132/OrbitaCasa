...
def _immobil_crea_tab(self, nb, imm, db, win):
    ...
    def limita_importo(*args):
        ...
        virgole = [i for i, c in enumerate(testo_filtrato) if c == ","]
        if len(virgole) > 1:
            idx_valido = virgole[0]
            testo_filtrato = "".join(
        ...
        if testo_filtrato and testo_filtrato != ",":
            try:
                valore_float = float(testo_filtrato.replace(",", "."))
    ...
    cols = ("Data", "Categoria", "Descrizione", "Tipo", "Importo", "Esportato")
    ...
    def _carica_in_form():
        ...
        try:
            ...
            v_imp.set(f"{val_float:.2f}".replace(".", ","))
        except ValueError:
    ...
    def _aggiungi():
        ...
        tipo  = v_tipo.get()
        testo_importo = v_imp.get().strip().replace(",", ".")
    ...
    def _esporta_in_spesedb():
        ...
        desc_export = f"Saldo {nome}" if len(spese_filtrate) <= 1 else f"Saldo {nome} ({len(spese_filtrate)} mov.)"
        ...
        self.spese[oggi].append(SpesaEntry.nuova(
            cat_export, desc_export, imp_mov, tipo_mov,
            conto=(nome_conto if nome_conto and nome_conto != "(nessuno)" else ""),
            hashtag=["#immobili", f"#immexp:{export_id}"]
        ))
        ...
        _popola_tree()
        self.refresh_gui()
        self.show_toast(f"Saldo {nome} ({tipo_mov} {_fmt_it(imp_mov)}€) esportato.")
    def _esporta_singole_in_spesedb():
        ...
        for m in spese_filtrate:
            ...
            if imp == 0:
                continue
            tipo_mov = "Entrata" if m.get("tipo", "").lower() == "entrata" else "Uscita"
            ...
            export_id = str(uuid.uuid4())
            self.spese[oggi].append(SpesaEntry.nuova(
                cat_export, desc_export, imp, tipo_mov,
                conto=(nome_conto if nome_conto and nome_conto != "(nessuno)" else ""),
                hashtag=["#immobili", f"#immexp:{export_id}"]
            ))
            m["esportato"] = export_id
    ...
    tk.Label(riga_export1, text="(saldo unico delle righe selezionate)",
...
def _immobil_grafici(self, db):
    ...
    def _disegna(event=None):
        ...
        lbl_ent.config(text=f"Totale Entrate: € {_fmt_it(tot_ent)}")
        lbl_usc.config(text=f"Totale Uscite: € {_fmt_it(tot_usc)}")
        lbl_saldo.config(
            text=f"Saldo: € {_fmt_it(saldo)}",
        ...
        for i in range(1, 5):
            ...
            canvas.create_text(x_g, margin_t + plot_h + 4,
                               text=f"{_fmt_it(v_g, ',.0f')}", anchor="n",
        ...
        for i, (etichetta, valore) in enumerate(bars_data):
            ...
            canvas.create_text(x1 + 5, int(cy),
                               text=f"€ {_fmt_it(valore)}", anchor="w",
...
def _immobil_estratto(self, db, nb, v_fmese=None, v_fanno=None):
    ...
    lines += [
        ...
        f"  Canone    : € {_fmt_it(imm.get('canone', 0))}/mese{'' :<13} Scadenza  : {imm.get('scadenza_contratto', '─'):<26}",
    ...
    for m in spese:
        ...
        if tipo.lower() == "entrata":
            ...
            tot_usc += imp
            segno = "-"
        str_importo = f"{segno}{_fmt_it(imp)}"
    ...
    lines += [
        ...
        f"─────────────────────".rjust(70),
        f"Totale Entrate: {_fmt_it(tot_ent, '>15,.2f')} €".rjust(70),
        f"Totale Uscite:  {_fmt_it(tot_usc, '>15,.2f')} €".rjust(70),
        f"Saldo Netto:    {_fmt_it(saldo, '>15,.2f')} €".rjust(70),
        "",
        linea_doppia,
...
def _immobil_estratto_totale(self, db, v_fmese=None, v_fanno=None):
    ...
    lines += [
        ...
        f"  ────────────────────────────".rjust(100),
        f"  Totale Entrate Generali: {_fmt_it(tot_ent, '>15,.2f')} €".rjust(100),
        f"  Totale Uscite Generali:  {_fmt_it(tot_usc, '>15,.2f')} €".rjust(100),
        f"  Saldo Netto Totale:      {_fmt_it(saldo, '>15,.2f')} €".rjust(100),
        "",
        linea_doppia,
