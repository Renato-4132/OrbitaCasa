...
def _immobil_crea_tab(self, nb, imm, db, win):
    ...
    r += 1
    tk.Label(form_lf, text="Metodo:", bg=self.COLOR_WIDGET_BG, fg=self.COLOR_HEADER,
             font=("Arial", 9, "bold")).grid(row=r, column=0, sticky="w", padx=6, pady=3)
    v_metodo = tk.StringVar(value="")
    _metodi = ["","💰 Contanti","🔄 RID/SDD","🏦 Bonifico","💎 C.Credito",
               "💳 C.Debito","📶 Contactless","📯 PayPal","📮 Bollettino",
               "🏪 Prepagata","🪙 Assegno","🔘 Revolut","🍎 Apple Pay",
               "🎯 Google Pay","🏣 Postepay","📲 Satispay","🔀 Scalapay",
               "🛒 Amazon Pay"]
    cb_metodo = ttk.Combobox(form_lf, textvariable=v_metodo, values=_metodi,
                             state="readonly", style="Border.TCombobox", width=16)
    cb_metodo.grid(row=r, column=1, sticky="ew", padx=6, pady=3)
    r += 1
    ...
    _simboli_metodo = {
        ...
        "💎 C.Credito":"💎","💳 C.Debito":"💳","📶 Contactless":"📶",
        "📯 PayPal":"📯","📮 Bollettino":"📮","🏪 Prepagata":"🏪","🪙 Assegno":"🪙",
        "🔘 Revolut":"🔘","🍎 Apple Pay":"🍎","🎯 Google Pay":"🎯",
        "🏣 Postepay":"🏣","📲 Satispay":"📲","🔀 Scalapay":"🔀",
        "🛒 Amazon Pay":"🛒",
    }
    def _aggiorna_simbolo_desc(event=None):
        desc = v_desc.get().strip()
        for sim in _simboli_metodo.values():
            if desc.startswith(sim):
                desc = desc[len(sim):].lstrip()
                break
        nuovo = _simboli_metodo.get(v_metodo.get(), "")
        ent_desc.delete(0, tk.END)
        ent_desc.insert(0, f"{nuovo} {desc}".strip() if nuovo else desc)
        ent_desc.icursor(tk.END)
    cb_metodo.bind("<<ComboboxSelected>>", _aggiorna_simbolo_desc)
    ...
    def _carica_in_form():
        ...
        desc_sel = vals[2]
        tipo_sel = vals[3]
        imp_sel  = vals[4].replace(" €", "").strip()
        movimento = next((
            m for m in imm.get("spese", [])
            if m.get("data","") == data_sel
            and m.get("categoria","") == cat_sel
            and m.get("descrizione","") == desc_sel
            and m.get("tipo","") == tipo_sel
        ), None)
        if not movimento:
