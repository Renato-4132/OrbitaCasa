...
from moduli.modello_spesa import SIMBOLI_METODO
    ...
    for data_obj in sorted(self.spese.keys()):
        ...
        for idx_v, voce in enumerate(voci):
            try:
                ...
                desc_str = str(voce[1]).strip()
                imp      = float(voce[2])
                tipo_mov = str(voce[3]).strip()
            except Exception:
                continue
            if tipo_mov != "Uscita":
                continue
    ...
    def ricarica(*_):
        ...
        for data, voci in self.spese.items():
            ...
            for v in voci:
                if len(v) < 4:
                    continue
                categoria   = str(v[0]) if v[0] else "Varie"
                descrizione = str(v[1])
                try:
                    imp = float(v[2])
