...
def _patch_fitz_glifi_pdf():
    ...
    _DIR_FONT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets", "fonts")
    _FONT_REGOLARE  = os.path.join(_DIR_FONT, "DejaVuSans.ttf")
    _FONT_GRASSETTO = os.path.join(_DIR_FONT, "DejaVuSans-Bold.ttf")
    _FONT_MONOSPAZIO = os.path.join(_DIR_FONT, "DejaVuSansMono.ttf")

    if not (os.path.exists(_FONT_REGOLARE) and os.path.exists(_FONT_GRASSETTO) and os.path.exists(_FONT_MONOSPAZIO)):
        print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] ATTENZIONE: font Unicode non trovati in moduli/assets/fonts/")
        return

    _MAPPA_FONT_BASE14 = {
        "helv":              ("OC-Regolare",   _FONT_REGOLARE),
        "helvetica":         ("OC-Regolare",   _FONT_REGOLARE),
        "hebo":              ("OC-Grassetto",  _FONT_GRASSETTO),
        "helvetica-bold":    ("OC-Grassetto",  _FONT_GRASSETTO),
        "cour":              ("OC-Mono",       _FONT_MONOSPAZIO),
        "courier":           ("OC-Mono",       _FONT_MONOSPAZIO),
    }

    def _font_sostitutivo(fontname):
        chiave = (fontname or "helv").strip().lower()
        return _MAPPA_FONT_BASE14.get(chiave)

    _insert_text_originale = fitz.Page.insert_text
    def _insert_text_patchato(self, point, text, *args, fontname="helv", fontfile=None, **kwargs):
        if fontfile is None:
            sostituto = _font_sostitutivo(fontname)
            if sostituto:
                fontname, fontfile = sostituto
        return _insert_text_originale(self, point, text, *args, fontname=fontname, fontfile=fontfile, **kwargs)
    fitz.Page.insert_text = _insert_text_patchato
    _insert_textbox_originale = fitz.Page.insert_textbox
    def _insert_textbox_patchato(self, rect, buffer, *args, fontname="helv", fontfile=None, **kwargs):
        if fontfile is None:
            sostituto = _font_sostitutivo(fontname)
            if sostituto:
                fontname, fontfile = sostituto
        return _insert_textbox_originale(self, rect, buffer, *args, fontname=fontname, fontfile=fontfile, **kwargs)
    fitz.Page.insert_textbox = _insert_textbox_patchato
    _save_originale = fitz.Document.save
    def _save_patchato(self, *args, **kwargs):
        try:
            self.subset_fonts()
        except Exception:
            pass
        return _save_originale(self, *args, **kwargs)
    fitz.Document.save = _save_patchato
    print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Font Unicode PDF attivi")

_patch_fitz_glifi_pdf()
...
def registra_tutti_i_moduli(GestioneSpese):
    ...
    from moduli.spese_pianificate import (
        ...
        piani_puliti, on_piano_doppio_click,
    )
    ...
    from moduli.gamification import (
        ...
        annulla_azione_gamification, aggiorna_badge_header, mostra_dettaglio_gamification,
