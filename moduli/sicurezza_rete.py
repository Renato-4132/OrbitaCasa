...
def mostra_log_accessi(self):
    if hasattr(self, '_log_accessi_popup') and self._log_accessi_popup and self._log_accessi_popup.winfo_exists():
        self._log_accessi_popup.lift()
        self._log_accessi_popup.focus_force()
        return
    import __main__ as _app
    LOGIN_WEB = _app.LOGIN_WEB
    LOGIN_WEB_FAIL = _app.LOGIN_WEB_FAIL
    ...
    from datetime import datetime
    popup = tk.Toplevel(self)
    self._log_accessi_popup = popup
    popup.bind("<Destroy>", lambda e: setattr(self, '_log_accessi_popup', None)
