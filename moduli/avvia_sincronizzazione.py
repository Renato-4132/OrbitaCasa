                    ...
                    try:
                                ...
                                try:
                                    ...
                                    if not esiste_mov:
                                        ...
                                        with open(LOG_IMPORTAZIONI, "a", encoding="utf-8") as log:
                                            log.write(f"{datetime.now().strftime('%d/%m/%Y %H:%M'):<17} | {'ESTRATTO':<8} | {data_mov.strftime('%d/%m/%Y'):<10} | {d['desc']:<50} | {_app._fmt_it(abs(importo_mov)):>10} € | {tipo:<7} | {cat}\n")
                        ...
                        for d_chiave in self.spese:
                            for s in self.spese[d_chiave]:
                                if s[1] in (desc, f"ALL· {desc}") and abs(s[2] - abs(importo)) < 0.01:
                                    esiste = True
                                    break
                            if esiste: break
                        if not esiste:
                            desc_spesa = f"ALL· {desc}" if pdf_data else desc
                            self.spese.setdefault(data_dt, []).append(SpesaEntry.nuova(CATEGORIA_TEMPORANEA, desc_spesa, abs(importo), direzione))
                            self.operazioni_scaricate_sessione += 1
                            with open(LOG_IMPORTAZIONI, "a", encoding="utf-8") as log:
                                log.write(f"{datetime.now().strftime('%d/%m/%Y %H:%M'):<17} | {'MAIL':<8} | {data_dt.strftime('%d/%m/%Y'):<10} | {desc_spesa:<50} | {_app._fmt_it(abs(importo)):>10} € | {direzione:<7} | {CATEGORIA_TEMPORANEA}\n")
                            if pdf_data:
