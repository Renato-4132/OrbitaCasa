...
def avvia_sincronizzazione(self, manuale=False):
    ...
    try:
                ...
                if isinstance(response_part, tuple):
                    ...
                    try:
                            ...
                            for d in json.loads(raw_e):
                                ...
                                for s in self.spese.get(data_mov, []):
                                    if s[1] == d["desc"] and abs(s[2] - abs(importo_mov)) < 0.01:
                                        ...
                                        break
                                if not esiste_mov:
                                    self.spese.setdefault(data_mov, []).append((cat, "🤖 " + d["desc"], abs(importo_mov), tipo))
                        ...
                        if not esiste:
                            ...
                            self.spese.setdefault(data_oggi, []).append((CATEGORIA_TEMPORANEA, desc_spesa, importo, direzione))
                            self.operazioni_scaricate_sessione += 1
                            with open(LOG_IMPORTAZIONI, "a", encoding="utf-8") as log:
        ...
        if self.operazioni_scaricate_sessione > 0:
            self.riproduci_beep()
        if popup: popup.destroy()
        if movimenti_accumulati:
            self.after(0, lambda mv=movimenti_accumulati: self.apri_finestra_revisione_universale(mv))
