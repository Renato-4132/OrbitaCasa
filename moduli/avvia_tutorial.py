    ...
    passi = [
        ...
        },
        {
            ...
            "punti": [
                ...
                "Studio Professionale — clienti, fatture, magazzino",
    ...
    def _scarica_immagini():
        for i, p in enumerate(passi):
            ...
            try:
                ...
                stato["imgs"][i] = photo
                if stato["passo"] == i and win.winfo_exists():
                    win.after(0, lambda ph=photo: (
                        lbl_screenshot.config(image=ph, text=""),
                        setattr(lbl_screenshot, "image", ph)
                    ))
            except Exception:
                stato["imgs"][i] = None
    import threading
