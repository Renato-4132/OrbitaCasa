...
def mostra_grafici_fairshare(self, anno_sel="Tutti", mese_sel="Tutti"):
    ...
    nomi_p = sorted([f"{_ico_g(n)} {n}" for n in nomi_p_raw], key=lambda x: x.split(" ", 1)[1].lower())
    ...
    def _posiziona_tip(win_tip, ev):
        win_tip.update_idletasks()
        tw = win_tip.winfo_reqwidth()
        th = win_tip.winfo_reqheight()
        win_x0 = popup.winfo_rootx()
        win_y0 = popup.winfo_rooty()
        win_x1 = win_x0 + popup.winfo_width()
        win_y1 = win_y0 + popup.winfo_height()
        tx, ty = ev.x_root + 14, ev.y_root + 14
        if tx + tw > win_x1:
            tx = win_x1 - tw
        if tx < win_x0:
            tx = win_x0
    ...
    def _show_tip(event, canvas, data_dict):
        ...
        if tag_trovato == _tip_state["tag"] and _tip_state["win"] is not None:
            try:
                _posiziona_tip(_tip_state["win"], event)
                return
            except Exception:
                pass
        _hide_tip()
        ...
        ttk.Label(win, text=data_dict[tag_trovato], style="Tooltip.TLabel",
                  justify="left", font=("Courier New", 9, "bold")).pack()
        _posiziona_tip(win, event)
        _tip_state["win"] = win
    ...
    def disegna_tab1(event=None):
        ...
        for i in range(6):
            ...
            cv1.create_text(pad_l - 6, gy, text=f"{_fmt_it(val, ',.0f')}€", fill="#888", font=("Arial", 7), anchor="e")
        ...
        for i, nome in enumerate(nomi_g):
            ...
            _cats_items = sorted(cat_per_persona.get(nome, {}).items(),
                                  key=lambda x: x[1], reverse=True)
            if _cats_items:
                _larg_cat = max(len(c) for c, _ in _cats_items) + 1
                cats_txt = "\n".join(f"  {(c + ':').ljust(_larg_cat + 2)}{_fmt_it(v)} €"
                                      for c, v in _cats_items)
            else:
                cats_txt = ""
            cv1.create_rectangle(cx-bar_w-4, dy1, cx-4, base_y, fill=GIALLO, outline="", tags=(tag_d,))
            _tooltip_data1[tag_d] = f"{nome}\nDovuto: {_fmt_it(dov)} €\nResiduo: {_fmt_it(res)} €\n{cats_txt}"
            if dov > 0:
                cv1.create_text(cx-bar_w//2-4, dy1-9, text=f"{_fmt_it(dov, ',.0f')}€", fill=GIALLO, font=("Arial", 8, "bold"))
    ...
    def disegna_tab2(event=None):
        ...
        area_h = H - pad_t - pad_b
        base_y = pad_t + area_h
        max_v  = max(max(cat_tot_imp.values()), 1) if cat_tot_imp else 1
        scala  = area_h / max_v
        ...
        for i in range(6):
            ...
            cv2.create_text(pad_l - 6, gy, text=f"{_fmt_it(val, ',.0f')}€", fill="#888", font=("Arial", 7), anchor="e")
        ...
        for i, cat in enumerate(cats):
            ...
            deb_txt = "\n".join(f"  {n}: {_fmt_it(v)} €"
                for n, v in sorted(cat_debitori.get(cat, {}).items(),
                                   key=lambda x: x[1], reverse=True))
            tip_a = f"{cat}\nAperto (da saldare): {_fmt_it(ap)} €"
            if deb_txt:
                tip_a += f"\nIn attesa da:\n{deb_txt}"
            cred_txt = "\n".join(f"  {n}: {_fmt_it(v)} €"
                for n, v in sorted(cat_creditori.get(cat, {}).items(),
                                   key=lambda x: x[1], reverse=True))
            tip_c = f"{cat}\nChiuso (saldato): {_fmt_it(cl)} €"
            if cred_txt:
                tip_c += f"\nSaldato da:\n{cred_txt}"
            ay1 = base_y - ap * scala
            ...
            if ap > 0:
                cv2.create_text(cx-bar_w//2-4, ay1-9, text=f"{_fmt_it(ap, ',.0f')}€", fill=ROSSO, font=("Arial", 7, "bold"))
            cy1 = base_y - cl * scala
            cv2.create_rectangle(cx+4, cy1, cx+bar_w+4, base_y, fill=VERDE, outline="", tags=(tag_c,))
            _tooltip_data2[tag_c] = tip_c
            if cl > 0:
                cv2.create_text(cx+bar_w//2+4, cy1-9, text=f"{_fmt_it(cl, ',.0f')}€", fill=VERDE, font=("Arial", 7, "bold"))
    ...
    def disegna_tab3(event=None):
        ...
        for i in range(6):
            ...
            cv3.create_text(pad_l - 6, gy, text=f"{_fmt_it(val, ',.0f')}€", fill="#888", font=("Arial", 7), anchor="e")
