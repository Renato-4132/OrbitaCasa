...
def quick_add(self, event):
    try:
        ...
        entrate_mese = sum(float(e[2]) for d, entries in self.spese.items() 
            if d.month == data_sel.month and d.year == data_sel.year 
            for e in entries if e[3] == "Entrata")
        uscite_mese = sum(float(e[2]) for d, entries in self.spese.items() 
            if d.month == data_sel.month and d.year == data_sel.year 
            for e in entries if e[3] == "Uscita")
        diff_mese = entrate_mese - uscite_mese
        impegni_futuri = sum(float(e[2]) for d, entries in self.spese.items() 
