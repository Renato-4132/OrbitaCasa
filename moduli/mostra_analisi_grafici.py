...
def mostra_analisi_grafici(self):
    ...
    def disegna_barre(canvas, dati, colori, mostra_anno=False, mostra_tipo=False, centro=False):
        ...
        for i, (etichetta, valore) in enumerate(elementi):
            ...
            if valore >= 0:
                ...
                canvas.create_text((x0 + x1) / 2, y1 - 10, text=f"{int(valore)}", font=("Arial", 8), fill=self.TEXT_COLOR)
            else:
    ...
    def disegna_barre_categorie(canvas, dati, colori):
        ...
        for i, (categoria, valore) in enumerate(dati):
            ...
            canvas.create_text((x0 + x1) / 2, y1 - 12, text=f"{valore:,.2f}", font=("Arial", 9), fill=self.TEXT_COLOR)
        ...
        lbl_totale = tk.Label(canvas, text=f"Totale uscite: € {totale:,.2f}", font=("Arial", 10, "bold"), bg=self.COLOR_WIDGET_BG, fg=self.TEXT_COLOR, padx=5)
    ...
    def aggiorna_legenda_treeview(tree, dati, colori):
            ...
            for cat, val in dati_ordinati:
                    ...
                    tree.insert("", "end", values=(cat, f"€ {val:,.2f}"), tags=(tag_name,))
                    try:
    ...
    def aggiorna_tab3(event=None):
        ...
        if saldo_totale >= 0:
            self.lbl_saldo_tab3.config(text=f"Saldo: € {saldo_totale:,.2f}", style="GSaldoPositivo.TLabel")
        else:
            self.lbl_saldo_tab3.config(text=f"Saldo: € {saldo_totale:,.2f}", style="GSaldoNegativo.TLabel")
    ...
    def aggiorna_tab1(event=None):
        ...
        if saldo >= 0:
            lbl_saldo_tab1.config(text=f"Saldo: € {saldo:,.2f}", style="GSaldoPositivo.TLabel")
        else:
            lbl_saldo_tab1.config(text=f"Saldo: € {saldo:,.2f}", style="GSaldoNegativo.TLabel")
        grafico = {}
        colore = {}
        if anno_selezionato == "Tutti":
