...
if not os.path.isfile(_COSTANTI_PATH):
    print(f"[{time.strftime('%H:%M:%S')}] Prima esecuzione: scarico costanti.py da GitHub...")
    try:
        os.makedirs(MODULI_DIR, exist_ok=True)
        _url = "https://raw.githubusercontent.com/Renato-4132/OrbitaCasa/main/moduli/costanti.py"
        ...
        with urllib.request.urlopen(_req, timeout=20) as resp:
            _dati = resp.read()
        with open(_COSTANTI_PATH, "wb") as f:
            f.write(_dati)
        print(f"[{time.strftime('%H:%M:%S')}] costanti.py scaricato.")
    except Exception as _e:
        print(f"[{time.strftime('%H:%M:%S')}] Errore durante il download di costanti.py: {_e}")
        show_warning_popup(
                    titolo="ATTENZIONE", titolo_fg="red",
                    corpo="Non è presente una connessione internet.\nImpossibile installare le risorse necessarie.\nIl programma verrà chiuso.",
                    corpo_fg="#61AFEF", corpo_font_size=11, corpo_expand=True,
...
class GestioneSpese(tk.Tk):
    ...
    def __init__(self):
        ...
        icona_temp = self.icone_gui.get("meteo_temporale")
        ...
        def on_spese_mese_tree_right_click(event):
            ...
            valori = self.spese_mese_tree.item(item, "values")
            if not valori or len(valori) < 5:
                return
            categoria   = str(valori[1]).strip()
            descrizione = str(valori[2]).strip()
            importo_str = str(valori[3]).replace("€", "").replace(",", ".").strip()
        ...
        metodi = ["", "📂 Movimenti", "───────────", "💰 Contanti", "🔄 RID/SDD", "🏦 Bonifico", "💎 C.Credito", "💜 C.Credito", "💳 C.Debito", "📶 Contactless", "📯 PayPal", "📮 Bollettino", "🏪 Prepagata", "🪙 Assegno", "🔘 Revolut", "🍎 Apple Pay", "🎯 Google Pay", "🏣 Postepay", "📲 Satispay", "🔀 Scalapay", "🛒 Amazon Pay"]
        ...
        def _get_tutti_tag():
            if not hasattr(self, '_cache_tutti_tag'):
                tdb = self._carica_tags_db()
                tutti = set()
                for lst in tdb.values():
                    for t in lst:
                        tutti.add(t.lstrip("#"))
    ...
    def _bind_tooltip_metodo(self, tree, col_desc=2):
        ...
        def _mostra(event):
            ...
            if not valori or len(valori) <= col_desc:
                return
            ...
            def _crea():
                ...
                ttk.Label(_tt[0], text=f"Pagamento: {metodo}", style="Tooltip.TLabel").pack()
                x = self.winfo_pointerx() + 12
                y = self.winfo_pointery() + 12
                _tt[0].wm_geometry(f"+{x}+{y}")
                _tt[0].deiconify()
    ...
    def _carica_tags_db(self):
        try:
            if os.path.exists(TAGS_DB):
                with open(TAGS_DB, "r", encoding="utf-8") as f:
                    return json.load(f)
        except Exception:
            pass
        return {}
    def _salva_tags_db(self, db):
        try:
            with open(TAGS_DB, "w", encoding="utf-8") as f:
    ...
    def add_ricorrenza(self, event=None):
        ...
        self.save_db()
        if hasattr(self, 'v_conto_ricorrenza'):
            _nome_c = self.v_conto_ricorrenza.get()
            if _nome_c and _nome_c != "(nessuno)":
                try:
                    with open(PORTAFOGLIO_BANCARIO, "r", encoding="utf-8") as _f:
                        _db_p = json.load(_f)
                    _conto = next((c for c in _db_p["conti"] if c.get("nome") == _nome_c), None)
                    if _conto:
                        _segno = -1 if tipo == "Uscita" else 1
                        _ids = {t.get("id","") for t in _db_p.get("trasferimenti",[])}
                        _i = 1
                        for _d in date_list:
                            _conto["saldo"] = round(float(_conto.get("saldo",0)) + _segno * imp, 2)
                            while f"t{_i}" in _ids:
                                _i += 1
                            _db_p.setdefault("trasferimenti",[]).append({
                                "id": f"t{_i}",
                                "data": _d.strftime("%d-%m-%Y"),
                                "da": _conto["id"] if tipo == "Uscita" else "__spese__",
                                "a":  "__spese__" if tipo == "Uscita" else _conto["id"],
                                "importo": round(imp, 2),
                                "note": f"{cat} – {desc}".strip(" –")
                            })
                            _ids.add(f"t{_i}")
                            _i += 1
                    with open(PORTAFOGLIO_BANCARIO, "w", encoding="utf-8") as _f:
                        json.dump(_db_p, _f, indent=2, ensure_ascii=False)
    ...
    def add_spesa(self, event=None):
        ...
        if _tags:
            _tdb = self._carica_tags_db()
            _tdb[self._chiave_tag(d, cat, desc, imp)] = _tags
            self._salva_tags_db(_tdb)
        ...
        if _nome_c and _nome_c != "(nessuno)":
            try:
                with open(PORTAFOGLIO_BANCARIO, "r", encoding="utf-8") as _f:
                    _db_p = json.load(_f)
                for _c in _db_p["conti"]:
                    if _c.get("nome") == _nome_c:
                        _segno = -1 if tipo == "Uscita" else 1
                        _c["saldo"] = round(float(_c.get("saldo", 0)) + _segno * imp, 2)
                        _ids = {t.get("id","") for t in _db_p.get("trasferimenti",[])}
                        _i = 1
                        while f"t{_i}" in _ids:
                            _i += 1
                ...
                if hasattr(self, '_saldo_popup') and self._saldo_popup and self._saldo_popup.winfo_exists():
                    self._saldo_refresh() if hasattr(self, "_saldo_refresh") else None; self._saldo_refresh_movimenti() if hasattr(self, "_saldo_refresh_movimenti") else None
            except Exception as _ex:
                self.show_toast(f"Errore aggiornamento conto: {_ex}")
        self.mostra_treeview_statistiche()
    ...
    def avvia_modifica_da_selezione(self):
        ...
        self.btn_tipo_spesa.config(style=new_style)
        if hasattr(self, 'v_conto_movimento'):
            self.v_conto_movimento.set(self._trova_conto_da_portafoglio(giorno, float(imp), tipo, self._ordine_in_gruppo_chiave(giorno, idx)))
        if hasattr(self, 'tag_entry'):
            self.tag_entry.delete(0, tk.END)
            _tdb = self._carica_tags_db()
            _key = self._chiave_tag(giorno, cat, desc, float(imp))
            _tags = _tdb.get(_key, [])
    ...
    def _aggiorna_conto_portafoglio(self, nome_conto, imp_old, tipo_old, imp_new, tipo_new, data_new, cat_new, desc_new, data_old=None, ordinale=0):
        if not nome_conto or nome_conto == "(nessuno)":
            return
        try:
            with open(PORTAFOGLIO_BANCARIO, "r", encoding="utf-8") as _f:
                _db_p = json.load(_f)
            if imp_old is not None and tipo_old is not None:
                _data_old_s = data_old.strftime("%d-%m-%Y") if data_old else None
                _candidati_idx = []
                for _idx_t, t in enumerate(_db_p.get("trasferimenti", [])):
                    if (_data_old_s is None or t.get("data") == _data_old_s) and \
                       abs(float(t.get("importo", 0)) - imp_old) < 0.01:
                        if (tipo_old == "Uscita" and t.get("a") in ("__spese__", "Contabilità")) or \
                           (tipo_old == "Entrata" and t.get("da") in ("__spese__", "Contabilità")):
                            _candidati_idx.append(_idx_t)
                if ordinale < len(_candidati_idx):
                    _idx_da_rimuovere = _candidati_idx[ordinale]
                    t = _db_p["trasferimenti"][_idx_da_rimuovere]
                    if tipo_old == "Uscita" and t.get("a") in ("__spese__", "Contabilità"):
                        for c in _db_p["conti"]:
                            if c["id"] == t.get("da"):
                                c["saldo"] = round(float(c.get("saldo", 0)) + imp_old, 2)
                    elif tipo_old == "Entrata" and t.get("da") in ("__spese__", "Contabilità"):
                        for c in _db_p["conti"]:
                            if c["id"] == t.get("a"):
                                c["saldo"] = round(float(c.get("saldo", 0)) - imp_old, 2)
                    del _db_p["trasferimenti"][_idx_da_rimuovere]
            if imp_new is not None and tipo_new is not None:
                conto = next((c for c in _db_p["conti"] if c.get("nome") == nome_conto), None)
                if conto:
                    segno_new = -1 if tipo_new == "Uscita" else 1
                    conto["saldo"] = round(float(conto.get("saldo", 0)) + segno_new * imp_new, 2)
                    _ids = {t.get("id", "") for t in _db_p.get("trasferimenti", [])}
                    _i = 1
                    while f"t{_i}" in _ids:
                        _i += 1
                    _db_p.setdefault("trasferimenti", []).append({
                        "id": f"t{_i}",
                        "data": data_new.strftime("%d-%m-%Y"),
                        "da": conto["id"] if tipo_new == "Uscita" else "__spese__",
                        "a":  "__spese__" if tipo_new == "Uscita" else conto["id"],
                        "importo": round(imp_new, 2),
                        "note": f"{cat_new} – {desc_new}".strip(" –")
                    })
            with open(PORTAFOGLIO_BANCARIO, "w", encoding="utf-8") as _f:
                json.dump(_db_p, _f, indent=2, ensure_ascii=False)
            self.refresh_gui()
    ...
    def salva_modifica(self):
        ...
        _ordinale_dup = self._ordine_in_gruppo_chiave(old_dt, idx)
        id_ric = voce_old[4] if len(voce_old) == 5 else None
        if "📎" in str(voce_old[1]):
                    ...
                    if os.path.exists(REGISTRY_FILE):
                        ...
                        for f_k in list(r.keys()):
                            if f_k.startswith(o_s) and o_i in f_k and o_t in f_k:
                                ...
                                val.update({'data_raw':n_s, 'categoria_esatta':cat, 'descrizione_esatta':f"📎 {d_p}", 'importo_raw':int(n_i), 'tipo_esatto':tipo})
                                r[n_f], ok = val, True
                                break 
                        if ok:
        ...
        _tags = self._normalizza_tags(self.tag_entry.get()) if hasattr(self, 'tag_entry') else []
        _tdb = self._carica_tags_db()
        _tdb.pop(self._chiave_tag(old_dt, voce_old[0], voce_old[1], voce_old[2]), None)
        if _tags:
            _tdb[self._chiave_tag(new_dt, cat, desc, imp)] = _tags
        self._salva_tags_db(_tdb)
        if hasattr(self, '_cache_tutti_tag'):
                del self._cache_tutti_tag
        if hasattr(self, 'tag_entry'):
        ...
        self.save_db()
        _nome_c = self.v_conto_movimento.get() if hasattr(self, 'v_conto_movimento') else "(nessuno)"
        _nome_c_old = self._trova_conto_da_portafoglio(old_dt, float(voce_old[2]), voce_old[3], _ordinale_dup)
        if _nome_c == _nome_c_old:
            if _nome_c and _nome_c != "(nessuno)":
                self._aggiorna_conto_portafoglio(
                    _nome_c,
                    float(voce_old[2]), voce_old[3],
                    imp, tipo, new_dt, cat, desc,
                    data_old=old_dt, ordinale=_ordinale_dup
                )
        else:
            if _nome_c_old and _nome_c_old != "(nessuno)":
                self._aggiorna_conto_portafoglio(
                    _nome_c_old,
                    float(voce_old[2]), voce_old[3],
    ...
    def cancella_voce(self):
        ...
        if old_dt in self.spese and 0 <= idx < len(self.spese[old_dt]):
            ...
            _nome_c = self.v_conto_movimento.get() if hasattr(self, 'v_conto_movimento') else "(nessuno)"
            _nome_c_old = self._trova_conto_da_portafoglio(old_dt, float(voce_old[2]), voce_old[3], _ordinale_dup)
            _conto_da_stornare = _nome_c if _nome_c and _nome_c != "(nessuno)" else _nome_c_old
            if _conto_da_stornare and _conto_da_stornare != "(nessuno)":
                self._aggiorna_conto_portafoglio(
                    _conto_da_stornare,
                    float(voce_old[2]), voce_old[3],
                    None, None, None, None, None,
    ...
    def draw_estratto_metodo(self):
            ...
            if d.year == view_year and d.month == view_month:
                for entry in entries:
                    ...
                    if not simbolo_trovato:
                        continue
                    nome_base = f"{simbolo_trovato} {_simboli_metodo[simbolo_trovato]}"
                    _key_conto = (d.strftime("%d-%m-%Y"), round(float(imp), 2), str(tipo).capitalize())
                    sotto_nome = _agganci_conto.get(_key_conto, "")
        ...
        for i, (metodo, val) in enumerate(righe_ordinate):
            ...
            c.tag_bind(tag_riga, "<Double-1>",
                       lambda e, filtri=simb_metodo, lbl=metodo, m=view_month, a=view_year:
        ...
        for d, entries in self.spese.items():
            ...
            if d.year == view_year and d.month == view_month:
                for entry in entries:
                    cat, desc, imp, tipo = entry[:4]
                    _key_conto = (d.strftime("%d-%m-%Y"), round(float(imp), 2), str(tipo).capitalize())
                    nome_conto = _agganci_conto.get(_key_conto, "")
                    if not nome_conto:
                        continue
                    segno = 1 if tipo == "Entrata" else -1
    ...
    def draw_heatmap_mese(self):
        ...
        for g_day in range(1, giorni + 1):
            ...
            if u > 0: c.create_text((x1+x2)/2, (y1+y2)/2 + 2, text=f"-{u:.0f}", fill=t_col, font=("Arial", 7))
            if e > 0: c.create_text((x1+x2)/2, y2-7, text=f"+{e:.0f}", fill=t_col, font=("Arial", 7))
            if g_day == now.day:
                c.create_rectangle(x1, y1, x2, y2, outline="#E5C07B", width=2)
            tips[g_day] = (
                f"{g_day:02d}/{view_month:02d}/{view_year}\n"
                f"{'Uscite:':<9}{('-'+f'{u:,.2f}'):>13} €\n"
                f"{'Entrate:':<9}{('+'+f'{e:,.2f}'):>13} €\n"
                f"{'Netto:':<9}{(('+' if netto>=0 else '')+f'{netto:,.2f}'):>13} €"
            )
        def _on_motion(e):
    ...
    def draw_spark_mese(self):
        ...
        def disegna_serie(valori, colore, tipo_label):
            ...
            for g, x, y in pts:
                ...
                if valori[g] > 0:
                    ...
                    tip = f"{data_str}  —  {tipo_label}: €{valori[g]:,.2f}"
                else:
                    tip = f"{data_str}\n{tipo_label}: nessuna"
        ...
        c.create_text(pad_l + 74, 9, text="Entrate", anchor="w",
                      fill=self.TEXT_COLOR, font=("Arial", 8))
        c.create_text(pad_l + 130, 9, text=f"(max €{max_val:,.0f})", anchor="w",
    ...
    def treeview_sort_column(self, tv, col, reverse):
        ...
        try:
            items.sort(
                key=lambda t: float(str(t[0]).replace(",", ".").replace("€", "").replace(" ", "") or 0),
                reverse=reverse
            )
        except Exception:
    ...
    # Aggiorna la tabella statistiche in base alla modalità (giorno/mese/anno/totali) calcolando entrate, uscite e differenza
    ...
    def _saldo_effettivo(self, conto, db):
        if self.considera_futuri_portafoglio_var.get():
            return float(conto.get("saldo", 0))
        oggi = datetime.date.today()
        ...
        for t in db.get("trasferimenti", []):
            try:
                ...
                if t.get("da") == conto["id"]:
                    saldo += imp
                elif t.get("a") == conto["id"]:
                    saldo -= imp
            except Exception:
                pass
        return saldo
            ...
            for idx, sp in enumerate(spese_giorno):
                try:
                    if isinstance(sp, dict):
                        ...
                        desc = sp.get("descrizione", "")
                        imp  = float(sp.get("importo", 0.0))
                        tipo = sp.get("tipo", "")
                    else:
                        cat  = sp[0] if len(sp) > 0 else ""
                        desc = sp[1] if len(sp) > 1 else ""
                        imp  = float(sp[2]) if len(sp) > 2 else 0.0
                        tipo = sp[3] if len(sp) > 3 else ""
    ...
    def salva_modifica_voce(self, params):
        ...
        _conto_form = params.get("conto", [""])[0].strip()
        if _conto_form and _conto_form != "(nessuno)":
            _nome_c_web = _conto_form
        else:
            _nome_c_web = self._trova_conto_da_portafoglio(d_vecchia_obj, _imp_old, _tipo_old)
        ...
        self.save_db()
        if _nome_c_web and _nome_c_web != "(nessuno)":
            try:
                self._aggiorna_conto_portafoglio(
                    _nome_c_web,
                    _imp_old, _tipo_old,
                    round(float(imp), 2), tipo, d_nuova_obj, cat, descr,
                    data_old=d_vecchia_obj
                )
            except Exception as _ex:
                print(f"[salva_modifica_voce] Errore pulizia portafoglio: {_ex}")
        self.carica_db_web()
        if hasattr(self, 'refresh_gui'):
            self.refresh_gui()
        return provenienza
    ...
    def _blink_recursive(self, label):
        ...
        try:
            ...
            print(f"Errore nel scheduling del timer. Controllare self.blinking_interval.")
            pass
    ...
    def verify_environment_update(self, tipo_install="UNKNOWN", rating=0):
        import datetime, hashlib, uuid, requests, platform, base64
        try:
...
# Backup Incrementale
def backup_incrementale(file_path, cartella_backup=None, max_backup=MAX_BACKUP):
...
def _rc():
    try:
        E_H_B = "ee7167e55d691b7eed90bd8be95660fdd83d33b9c0cd977ecc712ab23d6b85ea"
