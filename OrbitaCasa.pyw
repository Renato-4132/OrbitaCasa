...
class GestioneSpese(tk.Tk):
    ...
    def avvia_modifica_da_selezione(self):
        ...
        self.btn_tipo_spesa.config(style=new_style)
        if hasattr(self, 'v_conto_movimento'):
            self.v_conto_movimento.set(self._trova_conto_da_portafoglio(giorno, float(imp), tipo))
        if hasattr(self, 'tag_entry'):
            self.tag_entry.delete(0, tk.END)
            _tdb = self._carica_tags_db()
    ...
    def _trova_conto_da_portafoglio(self, data, imp, tipo):
        try:
            with open(PORTAFOGLIO_BANCARIO, "r", encoding="utf-8") as _f:
                _db_p = json.load(_f)
    ...
    def _aggiorna_conto_portafoglio(self, nome_conto, imp_old, tipo_old, imp_new, tipo_new, data_new, cat_new, desc_new, data_old=None):
        ...
        try:
            ...
            if imp_old is not None and tipo_old is not None:
                ...
                nuovi_tras = []
                conto_id = next((c["id"] for c in _db_p["conti"] if c.get("nome") == nome_conto), None)
                for t in _db_p.get("trasferimenti", []):
                    if (_data_old_s is None or t.get("data") == _data_old_s) and \
                       abs(float(t.get("importo", 0)) - imp_old) < 0.01 and \
                       (conto_id is None or t.get("da") == conto_id or t.get("a") == conto_id):
                        if tipo_old == "Uscita" and t.get("a") in ("__spese__", "Contabilità"):
                            ...
                            continue
                    nuovi_tras.append(t)
                _db_p["trasferimenti"] = nuovi_tras
    ...
    def salva_modifica(self):
        ...
        self.save_db()
        _nome_c = self.v_conto_movimento.get() if hasattr(self, 'v_conto_movimento') else "(nessuno)"
        _nome_c_old = self._trova_conto_da_portafoglio(old_dt, float(voce_old[2]), voce_old[3])
    ...
    def update_stats(self):
        ...
        if mode == "giorno":
            ...
            for idx, entry in enumerate(spese):
                ...
                nome_conto_st = _lista_conti[_uso] if _uso < len(_lista_conti) else (_lista_conti[-1] if _lista_conti else "")
        ...
        for g in range(1, num_giorni + 1):
            ...
            for idx, sp in enumerate(spese_giorno):
                try:
                    ...
                    conto = _lista[_uso] if _uso < len(_lista) else (_lista[-1] if _lista else "")
...
def _rc():
    try:
        E_H_B = "0445780e565ecc26060d2abc03b83019cd4fe887d7f9c89943d9f553249ab803"
