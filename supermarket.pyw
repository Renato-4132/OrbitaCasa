...
# Palette tema scuro (basta cambiare questi valori per personalizzare i colori)
COLOR_BACKGROUND = "#1e2128"
COLOR_WIDGET_BG = "#2a2e37"
COLOR_HEADER_BG = "#252932"
COLOR_HIGHLIGHT = "#5c99ff"
TEXT_COLOR = "#e6e6e6"
TEXT_MUTED = "#9aa0aa"
...
class ScontrinoParserApp:
    ...
    def _crea_interfaccia(self):
        ...
        self.file_label = ttk.Label(input_frame, text="Nessun file selezionato.", foreground=TEXT_MUTED)
        ...
        self.db_path_label = ttk.Label(db_controls_frame, text="DB: Non Caricato", foreground=COLOR_HIGHLIGHT)
        ...
        self.log_text = scrolledtext.ScrolledText(
            log_frame, wrap=tk.WORD, height=6, font=('Courier', 9),
            bg=COLOR_WIDGET_BG, fg=TEXT_COLOR, insertbackground=TEXT_COLOR,
            relief='flat', bd=0
        )
        self.log_text.pack(fill='both', expand=True)
        self.log_text.tag_config('error', foreground='#ff8080', background='#3a2020')
        self.log_text.tag_config('normal', foreground=TEXT_COLOR)
...
if __name__ == "__main__":
    ...
    # --- Tema scuro generale (sostituisce il grigio di default del tema 'clam') ---
    style.configure('.', background=COLOR_BACKGROUND, foreground=TEXT_COLOR,
                     fieldbackground=COLOR_WIDGET_BG, bordercolor=COLOR_HEADER_BG,
                     darkcolor=COLOR_BACKGROUND, lightcolor=COLOR_BACKGROUND)
    style.configure('TFrame', background=COLOR_BACKGROUND)
    style.configure('TLabel', background=COLOR_BACKGROUND, foreground=TEXT_COLOR)
    style.configure('TLabelframe', background=COLOR_BACKGROUND, bordercolor=COLOR_HIGHLIGHT)
    style.configure('TLabelframe.Label', background=COLOR_BACKGROUND, foreground=COLOR_HIGHLIGHT,
                     font=('Arial', 9, 'bold'))
    style.configure('TEntry', fieldbackground=COLOR_WIDGET_BG, foreground=TEXT_COLOR,
                     insertcolor=TEXT_COLOR)
    style.configure('TCombobox', fieldbackground=COLOR_WIDGET_BG, foreground=TEXT_COLOR)
    style.configure('TScrollbar', background=COLOR_HEADER_BG, troughcolor=COLOR_BACKGROUND,
                     bordercolor=COLOR_BACKGROUND, arrowcolor=TEXT_COLOR)

    style.configure("Custom.Treeview.Heading", font=('Arial', 8, 'normal'),
                     background=COLOR_HEADER_BG, foreground=TEXT_COLOR, relief='flat')
    style.map("Custom.Treeview.Heading", background=[('active', COLOR_HIGHLIGHT)])
    style.configure("Custom.Treeview", font=('Arial', 8, 'normal'), rowheight=18,
                     background=COLOR_WIDGET_BG, fieldbackground=COLOR_WIDGET_BG,
                     foreground=TEXT_COLOR, bordercolor=COLOR_HEADER_BG)
